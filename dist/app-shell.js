...
</svg>`},xK={ae:1.3333,ar:1.3333,cl:1.3333,cr:1.3333,gm:1.3333,gt:1.3333,hk:1.3333,nz:1.3333,sg:1.3333,tw:1.3333,us:1.3333};function oh(l){let h=LK[l.toLowerCase()];return h?"data:image/svg+xml,"+encodeURIComponent(h):""}var th={us:{aspect:1.3333333333333333},cn:{scale:1.2,dx:2.2,dy:2.8},kr:{scale:0.72}};var DK="M19.964 8.156 15.758.844A1.69 1.69 0 0014.299 0H5.887c-.6 0-1.156.32-1.456.844L.225 8.156c-.3.523-.3 1.165 0 1.688l4.206 7.312c.3.523.856.844 1.456.844h8.412c.6 0 1.156-.32 1.456-.844l4.206-7.312a1.69 1.69 0 00.003-1.688",O1=0;function T1(l,h){let Q=l?.aspect??(h?xK[h.toLowerCase()]??1:1),K=l?.scale??1,Z=l?.dx??0,V=l?.dy??0,W=1.08,M=Math.max(20,18*Q)*1.08*K,G=M/Q;return{x:10+Z-M/2,y:9+V-G/2,w:M,h:G}}function S1(l,h={}){let Q=h.size??24,K=h.fit??th[l.toLowerCase()],{x:Z,y:V,w:W,h:M}=T1(K,l),G=Math.max(0.4,22/Q),Y=`nq-flag-${O1+=1}`;return`<svg class="nq-flag-hex" viewBox="0 0 20 18" width="${Q}" height="${(Q*0.9).toFixed(2)}" aria-hidden="true" style="display:block;overflow:visible"><defs><clipPath id="${Y}"><path d="${DK}"/></clipPath></defs><g clip-path="url(#${Y})"><image href="${oh(l)}" x="${Z}" y="${V}" width="${W}" height="${M}" preserveAspectRatio="xMidYMid slice"/></g><path d="${DK}" fill="none" stroke="rgba(31,35,72,0.4)" stroke-width="${G.toFixed(2)}" stroke-linejoin="round"/></svg>`}function s(l,h={}){let Q=document.createElement("div");return Q.innerHTML=S1(l,h),Q.firstElementChild}function f1(l,h,Q,K={}){let Z=K.pad??2,V=K.bandDepth??14;if(l.length===0)return{line:"",fill:"",band:""};let W=Math.max(...l),M=Math.min(...l),G=W-M,Y=Q-Z*2,P=(J)=>l.length===1?h/2:J/(l.length-1)*h,X=(J)=>G===0?Z+Y/2:Z+Y-(J-M)/G*Y,z=l.map((J,E)=>[P(E),X(J)]),I=`M${z.map(([J,E])=>`${J.toFixed(1)},${E.toFixed(1)}`).join("L")}`,$=`${I}L${h.toFixed(1)},${Q}L0,${Q}Z`,C=z.slice().reverse().map(([J,E])=>`${J.toFixed(1)},${(E+V).toFixed(1)}`),L=`${I}L${C.join("L")}Z`;return{line:I,fill:$,band:L}}function Vh(l,h,Q,K){let Z=l*Math.PI*2-Math.PI/2;return[Q+h*Math.cos(Z),K+h*Math.sin(Z)]}function w1(l,h,Q,K){let Z=l.reduce((M,G)=>M+(G>0?G:0),0);if(Z<=0)return[];let V=[],W=0;return l.forEach((M,G)=>{if(M<=0)return;let Y=M/Z,[P,X]=Vh(W,h,Q,K),[z,j]=Vh(W+Y,h,Q,K),I=Y>=1?Vh(0.9999,h,Q,K):[z,j],$=Y>0.5?1:0;V.push({path:`M${P.toFixed(2)},${X.toFixed(2)}A${h},${h} 0 ${$} 1 ${I[0].toFixed(2)},${I[1].toFixed(2)}`,fraction:Y,index:G}),W+=Y}),V}var eh="data:image/svg+xml,"+encodeURIComponent('<svg width="64" height="64" viewBox="0 -4 64 64" xmlns="http://www.w3.org/2000/svg"><path opacity=".1" d="M62.3 25.4L49.2 2.6A5.3 5.3 0 0 0 44.6 0H18.4c-1.9 0-3.6 1-4.6 2.6L.7 25.4c-1 1.6-1 3.6 0 5.2l13.1 22.8c1 1.6 2.7 2.6 4.6 2.6h26.2c1.9 0 3.6-1 4.6-2.6l13-22.8c1-1.6 1-3.6.1-5.2z" fill="#1F2348"/></svg>');function y1(l){let h=(l??"").trim(),Q=h.split(/\s+/);if(Q.length>=5)return`${Q.slice(0,3).join(" ")} … ${Q.slice(-1)}`;if(h.length>16)return`${h.slice(0,8)}…${h.slice(-5)}`;return h}function R1(){if(typeof document>"u")return;if(document.getElementById("nimiq-shell-profile-style"))return;let l=document.createElement("style");l.id="nimiq-shell-profile-style",l.textContent=`
...
`,document.head.appendChild(l)}async function q1(l){if(!l)return eh;try{let Q=await import("@nimiq/iqons"),K=Q.default??Q;if(K&&typeof K.toDataUrl==="function")return await K.toDataUrl(l)}catch{}return eh}function lQ(l,h){let{wallet:Q,i18n:K}=h,Z=h.identiconSize??48,V=h.showDisconnect!==!1,W=h.showCopy!==!1;if(h.injectStyles!==!1)R1();let M=document.createElement("div");M.className="nq-profile",l.appendChild(M);let G=0;async function Y(){let z=++G,j=Q.account;M.textContent="";let I=document.createElement("span");I.className="nq-profile__icon",I.style.width=`${Z}px`,I.style.height=`${Z}px`;let $=null;if(j&&h.identicon)I.appendChild(h.identicon(j.address,Z));else $=document.createElement("img"),$.src=eh,$.alt="identicon",I.appendChild($);M.appendChild(I);let C=document.createElement("div");C.className="nq-profile__body";let L=document.createElement("span");if(L.className="nq-profile__label",L.textContent=j?j.label||K.t("shell.account"):K.t("shell.notConnected"),C.appendChild(L),j){let J=document.createElement("span");if(J.className="nq-profile__addr",J.textContent=y1(j.address),J.title=j.address,C.appendChild(J),h.getBalance){let E=document.createElement("span");E.className="nq-profile__bal",E.textContent="…",C.appendChild(E),Promise.resolve(h.getBalance(j.address)).then((N)=>{if(z===G)E.textContent=N}).catch(()=>{if(z===G)E.textContent=""})}}if(M.appendChild(C),j&&(W||V)){let J=document.createElement("div");if(J.className="nq-profile__actions",W){let E=document.createElement("button");E.type="button",E.className="nq-profile__btn",E.textContent=K.t("shell.copyAddress"),E.addEventListener("click",async()=>{try{await navigator.clipboard?.writeText(j.address);let N=E.textContent;E.textContent=K.t("shell.copied"),setTimeout(()=>{E.textContent=N},1200)}catch{}}),J.appendChild(E)}if(V){let E=document.createElement("button");E.type="button",E.className="nq-profile__btn",E.textContent=K.t("shell.disconnect"),E.addEventListener("click",()=>Q.disconnect()),J.appendChild(E)}M.appendChild(J)}if($){let J=await q1(j?.address??null);if(z===G)$.src=J}}Y();let P=Q.onAccountChange(()=>void Y()),X=K.onChange(()=>void Y());return{el:M,refresh(){Y()},destroy(){P(),X(),M.remove()}}}var OK="nimiq-shell-lang-switcher-style";function v1(){if(typeof document>"u")return;if(document.getElementById(OK))return;let l=document.createElement("style");l.id=OK,l.textContent=`
...
`,document.head.appendChild(l)}function B1(l,h){let{i18n:Q}=h,K=h.languages??rh,Z=h.size??40;if(h.injectStyles!==!1)v1();let V=document.createElement("ul");V.className="nq-langsw",V.setAttribute("role","listbox"),V.setAttribute("aria-label",Q.t("shell.language")),V.style.setProperty("--nq-flag-w",`${Z}px`);let W=new Map;for(let Y of K){let P=document.createElement("li"),X=document.createElement("button");X.type="button",X.className="nq-langsw__btn",X.setAttribute("role","option"),X.setAttribute("aria-label",Y.name);let z=document.createElement("span");z.className="nq-langsw__art",z.appendChild(s(Y.flag,{size:Z}));let j=document.createElement("span");j.className="nq-langsw__tip",j.setAttribute("aria-hidden","true"),j.textContent=Y.name,X.appendChild(z),X.appendChild(j),X.addEventListener("click",()=>Q.setLanguage(Y.id)),P.appendChild(X),V.appendChild(P),W.set(Y.id,X)}function M(Y){for(let[P,X]of W){let z=P===Y;X.classList.toggle("is-active",z),X.setAttribute("aria-selected",String(z))}}M(Q.getLanguage());let G=Q.onChange((Y)=>M(Y));return l.appendChild(V),{el:V,destroy(){G(),V.remove()}}}var TK="nimiq-shell-langpill-style";function b1(){if(typeof document>"u"||document.getElementById(TK))return;let l=document.createElement("style");l.id=TK,l.textContent=`
...
`,document.head.appendChild(l)}var g1='<svg class="nq-langpill__caret" viewBox="0 0 10 6" aria-hidden="true"><path d="M1 1l4 4 4-4" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>';function m1(l,h){let{i18n:Q}=h,K=h.languages??bl,Z=h.size??24;if(h.injectStyles!==!1)b1();let V=document.createElement("div");V.className="nq-langpill";let W=document.createElement("button");W.type="button",W.className="nq-langpill__btn",W.setAttribute("aria-haspopup","listbox"),W.setAttribute("aria-expanded","false"),W.setAttribute("aria-label",Q.t("shell.language"));let M=document.createElement("ul");M.className="nq-langpill__menu",M.setAttribute("role","listbox"),M.setAttribute("aria-label",Q.t("shell.language")),M.hidden=!0;let G=new Map;for(let C of K){let L=document.createElement("li"),J=document.createElement("button");J.type="button",J.className="nq-langpill__option",J.setAttribute("role","option"),J.appendChild(s(C.flag,{size:Z}));let E=document.createElement("span");E.className="nq-langpill__name",E.textContent=C.name,J.appendChild(E),J.addEventListener("click",()=>{Q.setLanguage(C.id),z()}),L.appendChild(J),M.appendChild(L),G.set(C.id,J)}V.appendChild(W),V.appendChild(M),l.appendChild(V);function Y(){let C=Q.getLanguage(),L=K.find((J)=>J.id===C)??K[0];if(W.textContent="",L)W.appendChild(s(L.flag,{size:Z}));W.insertAdjacentHTML("beforeend",g1)}function P(C){for(let[L,J]of G){let E=L===C;J.classList.toggle("is-active",E),J.setAttribute("aria-selected",String(E))}}function X(){M.hidden=!1,W.setAttribute("aria-expanded","true"),document.addEventListener("click",j,!0),document.addEventListener("keydown",I)}function z(){if(M.hidden)return;M.hidden=!0,W.setAttribute("aria-expanded","false"),document.removeEventListener("click",j,!0),document.removeEventListener("keydown",I)}function j(C){if(!V.contains(C.target))z()}function I(C){if(C.key==="Escape")z(),W.focus()}W.addEventListener("click",()=>M.hidden?X():z()),Y(),P(Q.getLanguage());let $=Q.onChange((C)=>{Y(),P(C)});return{el:V,destroy(){$(),z(),V.remove()}}}var SK="nimiq-shell-walletpill-style";function d1(){if(typeof document>"u"||document.getElementById(SK))return;let l=document.createElement("style");l.id=SK,l.textContent=`
...
`,document.head.appendChild(l)}var u1="data:image/svg+xml,"+encodeURIComponent('<svg width="64" height="64" viewBox="0 -4 64 64" xmlns="http://www.w3.org/2000/svg"><path opacity=".25" d="M62.3 25.4L49.2 2.6A5.3 5.3 0 0 0 44.6 0H18.4c-1.9 0-3.6 1-4.6 2.6L.7 25.4c-1 1.6-1 3.6 0 5.2l13.1 22.8c1 1.6 2.7 2.6 4.6 2.6h26.2c1.9 0 3.6-1 4.6-2.6l13-22.8c1-1.6 1-3.6.1-5.2z" fill="currentColor"/></svg>'),hQ='<svg class="nq-connect__icon" viewBox="0 0 20 20" fill="none" aria-hidden="true"><rect x="2" y="5" width="16" height="12" rx="2" stroke="currentColor" stroke-width="1.5"/><path d="M2 9h16" stroke="currentColor" stroke-width="1.5"/><circle cx="6" cy="13" r="1" fill="currentColor"/></svg>',c1='<svg class="nq-wallet__caret" viewBox="0 0 10 6" aria-hidden="true"><path d="M1 1l4 4 4-4" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>';function i1(l,h){if(l.label)return l.label;let Q=l.address?.trim()??"";if(!Q)return h;return`${Q.slice(0,7)}…${Q.slice(-4)}`}function p1(l,h){let{wallet:Q,i18n:K}=h;if(h.injectStyles!==!1)d1();let Z=document.createElement("div");Z.className="nq-wallet",l.appendChild(Z);let V=null,W=!1;function M(){V?.destroy(),V=null,document.removeEventListener("click",G,!0),document.removeEventListener("keydown",Y)}function G(C){if(!Z.contains(C.target))P()}function Y(C){if(C.key==="Escape")P()}function P(){if(!W)return;W=!1,j()}function X(){let C=document.createElement("button");C.className="nq-connect",C.type="button",C.innerHTML=hQ+`<span>${K.t("shell.connectWallet")}</span>`,C.addEventListener("click",async()=>{C.disabled=!0,C.innerHTML=hQ+`<span>${K.t("shell.connecting")}</span>`;try{await Q.connect()}catch{C.disabled=!1,C.innerHTML=hQ+`<span>${K.t("shell.retry")}</span>`}}),Z.appendChild(C)}function z(){let C=Q.account,L=document.createElement("button");L.className="nq-wallet__btn",L.type="button",L.setAttribute("aria-haspopup","dialog"),L.setAttribute("aria-expanded",String(W));let J=document.createElement("span");if(J.className="nq-wallet__icon",h.identicon)J.appendChild(h.identicon(C.address,28));else{let N=document.createElement("img");N.src=u1,N.alt="",J.appendChild(N)}L.appendChild(J);let E=document.createElement("span");if(E.className="nq-wallet__label",E.textContent=i1(C,K.t("shell.account")),L.appendChild(E),L.insertAdjacentHTML("beforeend",c1),L.addEventListener("click",()=>{W=!W,j()}),Z.appendChild(L),W){let N=document.createElement("div");N.className="nq-wallet__menu",Z.appendChild(N),V=lQ(N,{wallet:Q,i18n:K,identiconSize:40,identicon:h.identicon,showCopy:!0,showDisconnect:!0}),document.addEventListener("click",G,!0),document.addEventListener("keydown",Y)}}function j(){if(M(),Z.textContent="",Q.account)z();else X()}j();let I=Q.onAccountChange(()=>{W=!1,j()}),$=K.onChange(()=>{if(!Q.account)j()});return{el:Z,destroy(){I(),$(),M(),Z.remove()}}}var n1="https://wallet.nimiq.com";function fK(l,h={}){let Q=l.replace(/\s+/g,"").toUpperCase(),K=h.basePath??n1,Z=BigInt(h.amountLuna??0),V=h.message?.trim()??"",W=[Q];if(Z>0n||V)W.push(Z>0n?El(Z).toString():"");if(V)W.push(encodeURIComponent(V));let M=K.endsWith("/")?"":"/";return`${K}${M}#_request/${W.join("/")}_`}var Wh=[],QQ=[],wK=!1;function gl(l,h,Q){if(l.push(h),l.length>Q)l.shift()}function s1(l){try{return typeof l==="string"?l:JSON.stringify(l)}catch{return String(l)}}function yK(l){return l?`
`+String(l).split(`
`).slice(0,4).join(`
`):""}function RK(l){try{let h=new URL(l,location.href);return h.pathname+h.search}catch{return String(l).slice(0,120)}}function qK(l){let h=l.search(/[?#]/);return h===-1?l:l.slice(0,h)}function KQ(l){if(wK||typeof window>"u")return;wK=!0,window.addEventListener("error",(Q)=>{let K=Q.filename?`
  at ${Q.filename}:${Q.lineno}:${Q.colno}`:"";gl(Wh,(Q.message||"Error")+K+yK(Q.error?.stack),20)}),window.addEventListener("unhandledrejection",(Q)=>{let K=Q.reason??{};gl(Wh,`Unhandled rejection: ${K.message??String(Q.reason)}${yK(K.stack)}`,20)});let h=console.error.bind(console);if(console.error=(...Q)=>{try{gl(Wh,Q.map(s1).join(" "),20)}catch{}h(...Q)},typeof window.fetch==="function"){let Q=window.fetch,K=Q.bind(window),Z=async(V,W)=>{let M=typeof V==="string"?V:V instanceof URL?V.href:V.url,G=W?.method??(V instanceof Request?V.method:"GET"),Y=Boolean(l)&&M.startsWith(l);try{let P=await K(V,W);if(P.status>=400&&!Y)gl(QQ,`${G} ${RK(M)} → ${P.status}`,12);return P}catch(P){if(!Y)gl(QQ,`${G} ${RK(M)} → network error`,12);throw P}};window.fetch=Object.assign(Z,Q)}}function ZQ(l=!1){return{url:typeof location<"u"?qK(location.href):"",title:typeof document<"u"?document.title:"",referrer:typeof document<"u"?qK(document.referrer):"",userAgent:typeof navigator<"u"?navigator.userAgent:"",viewport:typeof window<"u"?{w:window.innerWidth,h:window.innerHeight,dpr:window.devicePixelRatio||1}:{w:0,h:0,dpr:1},consoleErrors:Wh.slice(-12),networkFailures:QQ.slice(-10),hasScreenshot:l}}var Fl=(l,h)=>`color-mix(in srgb, ${l} ${h}%, black)`,zQ=(l,h)=>`color-mix(in srgb, ${l} ${h}%, transparent)`,Mh=(l,h)=>`radial-gradient(100% 100% at 100% 100%, ${l}, ${h})`;function vK(l){let h={},Q=(V,W)=>{if(W)h[V]=W};if(Q("--nq-cc-font",l.font),l.surface)Q("--nq-cc-menu-bg",l.surface),Q("--nq-cc-card-bg",l.surface);if(l.text)Q("--nq-cc-menu-fg",l.text),Q("--nq-cc-menu-muted",zQ(l.text,60)),Q("--nq-cc-menu-hover",zQ(l.text,6)),Q("--nq-cc-menu-line",zQ(l.text,8));let K=l.face??l.surface,Z=l.faceText??l.text;if(Q("--nq-cc-face-bg",K),Q("--nq-cc-face-bg-hover",K),Q("--nq-cc-face-fg",Z),l.primary)Q("--nq-cc-connect-bg",l.primary),Q("--nq-cc-connect-image",Mh(Fl(l.primary,92),l.primary)),Q("--nq-cc-connect-image-hover",Mh(Fl(l.primary,Math.round(62.56)),Fl(l.primary,68)));if(l.accent)Q("--nq-cc-accent",l.accent),Q("--nq-cc-send-bg",l.accent),Q("--nq-cc-send-image",Mh(Fl(l.accent,92),l.accent)),Q("--nq-cc-send-image-hover",Mh(Fl(l.accent,Math.round(62.56)),Fl(l.accent,68)));if(Q("--nq-cc-connect-fg",l.primaryText),Q("--nq-cc-send-fg",l.accentText),Q("--nq-cc-danger",l.danger),Q("--nq-cc-success",l.success),l.warning)Q("--nq-cc-warning",l.warning),Q("--nq-cc-warn-bg",`color-mix(in srgb, ${l.warning} 12%, white)`),Q("--nq-cc-warn-line",`color-mix(in srgb, ${l.warning} 22%, white)`);return h}function Ul(l,h){let Q=vK(h);for(let K of Object.keys(Q))l.style.setProperty(K,Q[K])}var a1=["nimiq.cards","nimiq.casino","nimiq.cool","nimiq.gift","nimiq.gives","nimiq.kids","nimiq.life","nimiq.money","nimiq.multisend","nimiq.name","nimiq.ninja","nimiq.party","nimiq.sale","nimiq.software","nimiq.stream","nimiq.talk","nimiq.tax","nimiq.tips","nimiq.vote","nimiq.work","swellet"],r1={"swellet.app":"swellet","swellet.io":"swellet"};function GQ(l){let h=l.trim().toLowerCase().replace(/^www\./,"").replace(/\.$/,"");if(!h||h==="localhost"||h.endsWith(".local"))return null;if(/^\d+\.\d+\.\d+\.\d+$/.test(h)||!h.includes("."))return null;let Q=/^([a-z0-9-]+)\.(?:fly\.dev|pages\.dev|workers\.dev|vercel\.app|netlify\.app)$/.exec(h),K=r1[h]??(Q?Q[1].replace(/-/g,"."):h);return a1.includes(K)?K:null}var o1="https://bot.nimiq.tech";function ml(l){return l.replace(/NQ\d{2}[\s]?(?:[0-9A-HJ-NP-VXY]{4}[\s]?){8}/gi,"[address redacted]").replace(/(^|[^0-9a-f])([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})(?![0-9a-f])/gi,"$1[id redacted]")}function XQ(l){if(typeof l==="string")return ml(l);if(Array.isArray(l))return l.map(XQ);if(l&&typeof l==="object"){let h={};for(let[Q,K]of Object.entries(l))h[Q]=XQ(K);return h}return l}async function gK(l,h){let Q=(l.service??o1).replace(/\/$/,""),K=XQ({...h.pageContext??{},...h.context??{}}),Z=ml([`[${h.type}] ${h.title.trim()}`,"",h.description.trim(),...t1(h)].join(`
`)),V=async(W,M)=>{let G=await globalThis.fetch(`${Q}${W}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(M)}),Y={};try{Y=await G.json()}catch{}return{res:G,json:Y}};try{let W=await V("/api/draft",{repo:l.repo,text:Z,context:K});if(!W.res.ok)return{ok:!1,status:W.res.status,error:BK(W.json)};let M=W.json.draft,G=W.json.reportId;if(!M||!G)return{ok:!1,status:W.res.status,error:"The issue service sent back nothing to file."};let Y=[...new Set([...M.labels??[],...l.labels??[]])],P={reportId:G,repo:l.repo,title:ml(M.title),body:ml(M.body)},X=await V("/api/file",{...P,labels:Y});if(!X.res.ok&&Y.length&&String(X.json.error??"")==="github_failed")X=await V("/api/file",{...P,labels:[]});let z=X.json.url;if(!X.res.ok&&!z)return{ok:!1,status:X.res.status,error:BK(X.json)};return{ok:!0,status:X.res.status,issueNumber:X.json.number,issueUrl:z}}catch(W){return{ok:!1,status:0,error:W instanceof Error?W.message:String(W)}}}function t1(l){let h=l.context??{},Q=Object.entries(h).map(([K,Z])=>`${K}: ${Z}`).join(" · ");return Q?["","---",Q]:[]}function BK(l){let h=String(l.error??"");return{rate_limited:"Too many reports just now. Give it a minute.",unknown_repo:"The issue service doesn't know this app.",empty_report:"Please describe the problem first.",github_not_configured:"The issue service isn't connected to GitHub yet.",github_failed:"GitHub rejected the issue. Try again shortly.",already_filed:"That report was already filed."}[h]??"Something went wrong."}function mK(l){let h=["bug","idea","question"];if(!l.type||!h.includes(l.type))return"shell.fbErrType";if((l.title??"").trim().length<5)return"shell.fbErrTitle";if((l.description??"").trim().length<10)return"shell.fbErrDetails";return null}async function dK(l,h){let Q={...h.context??{},type:h.type,title:h.title.trim(),description:h.description.trim()};if(h.diagnostic)Q.diagnostic=h.diagnostic;let K;try{K=await globalThis.fetch(l,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(Q)})}catch(V){return{ok:!1,status:0,error:V instanceof Error?V.message:String(V)}}let Z={};try{Z=await K.json()}catch{}if(K.ok)return{ok:!0,status:K.status,issueNumber:Z.issueNumber};return{ok:!1,status:K.status,error:Z.error??`Server returned ${K.status}.`,fallbackMailto:Z.fallbackMailto}}function uK(){let l=[];try{if(typeof location<"u")l.push(`page: ${location.pathname}${location.hash}`);if(typeof navigator<"u")l.push(`ua: ${navigator.userAgent}`),l.push(`lang: ${navigator.language}`);if(typeof window<"u")l.push(`viewport: ${window.innerWidth}×${window.innerHeight}`)}catch{}return l.join(`
...
`,l.head.appendChild(h)}function u(l){return l.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function lZ(l,h,Q){let K=l.createElement("div");if(K.className="nq-fb-toast",K.setAttribute("role","status"),K.textContent=h,Q)Ul(K,Q);l.body.appendChild(K),setTimeout(()=>K.remove(),3000)}function kQ(l,h,Q){if(l.getElementById("nq-fb-scrim"))return;e1(l);let K=($)=>h.t($),Z=Q.diagnostics!==!1,V=l.createElement("div");if(V.id="nq-fb-scrim",V.className="nq-fb-scrim",Q.theme)Ul(V,Q.theme);V.innerHTML=`
    ...
    </div>`,l.body.appendChild(V);let W=($)=>V.querySelector(`#${$}`),M=W("nq-fb-type"),G=W("nq-fb-summary"),Y=W("nq-fb-details"),P=Z?W("nq-fb-diag"):null,X=W("nq-fb-error"),z=W("nq-fb-send"),j=($)=>{if($.key==="Escape")I()};function I(){V.remove(),l.removeEventListener("keydown",j)}W("nq-fb-cancel").addEventListener("click",I),V.addEventListener("pointerdown",($)=>{if($.target===V)I()}),l.addEventListener("keydown",j),G.focus();for(let $ of[M,G,Y])$.addEventListener("input",()=>{X.style.display="none"});z.addEventListener("click",async()=>{let $={type:M.value||"",title:G.value,description:Y.value,context:Q.context};if(P?.checked)if(Q.bot)$.pageContext=ZQ();else $.diagnostic=uK();let C=mK($);if(C){X.textContent=K(C),X.style.display="block";return}z.disabled=!0,z.textContent=K("shell.fbSending"),X.style.display="none";let L=Q.bot?await gK(Q.bot,$):await dK(Q.endpoint,$);if(z.disabled=!1,z.textContent=K("shell.fbSend"),L.ok){I(),lZ(l,K("shell.fbThanks"),Q.theme),Q.onSubmitted?.(L);return}let J=L.error??K("shell.fbFailed");X.innerHTML=L.fallbackMailto?`${u(J)} <a href="${u(L.fallbackMailto)}">${u(K("shell.fbFailEmail"))}</a>`:u(J),X.style.display="block"})}function hZ(l){let h=l.name?.trim(),Q=l.network.trim();if(!h)return Q;if(h.toLowerCase()===Q.toLowerCase())return h;return`${h} · ${Q}`}var cK="nimiq-shell-asset-list-style";function QZ(){if(typeof document>"u"||document.getElementById(cK))return;let l=document.createElement("style");l.id=cK,l.textContent=`
...
`,document.head.appendChild(l)}function ll(l,h,Q){let K=document.createElement(l);if(h)K.className=h;if(Q)Q.appendChild(K);return K}function VQ(l,h){if(h.injectStyles!==!1)QZ();let Q=h.cacheMs??30000,K=()=>typeof h.assets==="function"?h.assets():h.assets,Z=ll("div","nq-al");l.appendChild(Z);let V=new Map,W="",M=!1;function G(z){let j=z.map(($)=>`${$.ticker}${$.address?"+":"-"}`).join(" ");if(j===W)return;W=j,Z.textContent="";let I=new Map;for(let $ of z){let C=typeof h.onSelect==="function"&&!!$.address,L=ll(C?"button":"div","nq-al-row",Z);if(C)L.type="button",L.addEventListener("click",()=>h.onSelect($));let J=ll("span","nq-al-art",L);if($.icon)J.appendChild($.icon(26));let E=ll("span","nq-al-id",L),N=ll("span","nq-al-tick",E);N.textContent=$.ticker;let F=ll("span","nq-al-name",E);F.textContent=hZ($);let D=ll("span","nq-al-amt",L),w=ll("span","nq-al-units nq-al-pending",D);w.textContent="—";let T=ll("span","nq-al-fiat",D),O=V.get($.ticker),U={units:O?.units??null,fiat:O?.fiat??null,fetchedAt:O?.fetchedAt??0,unitsEl:w,fiatEl:T};if(I.set($.ticker,U),U.units!==null)Y($,U)}V.clear();for(let[$,C]of I)V.set($,C)}function Y(z,j){if(j.units===null)return;if(j.unitsEl.classList.remove("nq-al-pending"),j.unitsEl.textContent=Gh(j.units,z.decimals,{maxDecimals:z.maxDecimals??z.decimals}),j.fiat===null){j.fiatEl.textContent="";return}let I=h.fiatTicker?.()??"USD";try{j.fiatEl.textContent=Al(j.fiat,I)}catch{j.fiatEl.textContent=""}}async function P(z,j,I){let $=V.get(z.ticker);if(!$)return;if(!j&&$.units!==null&&I-$.fetchedAt<Q){Y(z,$);return}try{let C=await z.balance();if(M)return;if(C!==null&&C!==void 0)$.units=typeof C==="bigint"?C:BigInt(Math.round(C)),$.fetchedAt=I}catch{}if(M)return;if(Y(z,$),h.rate&&$.units!==null){try{let C=await h.rate(z.ticker);if(M)return;$.fiat=C===null?null:Number($.units)/10**z.decimals*C}catch{$.fiat=null}if(!M)Y(z,$)}}async function X(z=!1){if(M)return;let j=K();G(j);let I=Date.now();await Promise.all(j.map(($)=>P($,z,I)))}if(G(K()),h.autoRefresh!==!1)X();return{el:Z,refresh:X,total(){let z=null;for(let j of V.values()){if(j.fiat===null)continue;z=(z??0)+j.fiat}return z},units:(z)=>V.get(z)?.units??null,clear(){for(let z of V.values())z.units=null,z.fiat=null,z.fetchedAt=0,z.unitsEl.classList.add("nq-al-pending"),z.unitsEl.textContent="—",z.fiatEl.textContent=""},destroy(){M=!0,Z.remove()}}}var iK=null;class WQ{}WQ.render=function(l,h){iK(l,h)};self.QrCreator=WQ;(function(l){function h(M,G,Y,P){var X={},z=l(Y,G);z.u(M),z.J(),P=P||0;var j=z.h(),I=z.h()+2*P;return X.text=M,X.level=G,X.version=Y,X.O=I,X.a=function($,C){return $-=P,C-=P,0>$||$>=j||0>C||C>=j?!1:z.a($,C)},X}function Q(M,G,Y,P,X,z,j,I,$,C){function L(J,E,N,F,D,w,T){J?(M.lineTo(E+w,N+T),M.arcTo(E,N,F,D,z)):M.lineTo(E,N)}j?M.moveTo(G+z,Y):M.moveTo(G,Y),L(I,P,Y,P,X,-z,0),L($,P,X,G,X,0,-z),L(C,G,X,G,Y,z,0),L(j,G,Y,P,Y,0,z)}function K(M,G,Y,P,X,z,j,I,$,C){function L(J,E,N,F){M.moveTo(J+N,E),M.lineTo(J,E),M.lineTo(J,E+F),M.arcTo(J,E,J+N,E,z)}j&&L(G,Y,z,z),I&&L(P,Y,-z,z),$&&L(P,X,-z,-z),C&&L(G,X,z,-z)}function Z(M,G){var Y=G.fill;if(typeof Y==="string")M.fillStyle=Y;else{var{type:P,colorStops:X}=Y;if(Y=Y.position.map((j)=>Math.round(j*G.size)),P==="linear-gradient")var z=M.createLinearGradient.apply(M,Y);else if(P==="radial-gradient")z=M.createRadialGradient.apply(M,Y);else throw Error("Unsupported fill");X.forEach(([j,I])=>{z.addColorStop(j,I)}),M.fillStyle=z}}function V(M,G){l:{var{text:Y,v:P,N:X,K:z,P:j}=G;X=Math.max(1,X||1);for(z=Math.min(40,z||40);X<=z;X+=1)try{var I=h(Y,P,X,j);break l}catch(ul){}I=void 0}if(!I)return null;Y=M.getContext("2d"),G.background&&(Y.fillStyle=G.background,Y.fillRect(G.left,G.top,G.size,G.size)),P=I.O,z=G.size/P,Y.beginPath();for(j=0;j<P;j+=1)for(X=0;X<P;X+=1){var $=Y,C=G.left+X*z,L=G.top+j*z,J=j,E=X,N=I.a,F=C+z,D=L+z,w=J-1,T=J+1,O=E-1,U=E+1,m=Math.floor(Math.min(0.5,Math.max(0,G.R))*z),a=N(J,E),r=N(w,O),c=N(w,E);w=N(w,U);var hl=N(J,U);U=N(T,U),E=N(T,E),T=N(T,O),J=N(J,O),C=Math.round(C),L=Math.round(L),F=Math.round(F),D=Math.round(D),a?Q($,C,L,F,D,m,!c&&!J,!c&&!hl,!E&&!hl,!E&&!J):K($,C,L,F,D,m,c&&J&&r,c&&hl&&w,E&&hl&&U,E&&J&&T)}return Z(Y,G),Y.fill(),M}var W={minVersion:1,maxVersion:40,ecLevel:"L",left:0,top:0,size:200,fill:"#000",background:null,text:"no text",radius:0.5,quiet:0};iK=function(M,G){var Y={};if(Object.assign(Y,W,M),Y.N=Y.minVersion,Y.K=Y.maxVersion,Y.v=Y.ecLevel,Y.left=Y.left,Y.top=Y.top,Y.size=Y.size,Y.fill=Y.fill,Y.background=Y.background,Y.text=Y.text,Y.R=Y.radius,Y.P=Y.quiet,G instanceof HTMLCanvasElement){if(G.width!==Y.size||G.height!==Y.size)G.width=Y.size,G.height=Y.size;G.getContext("2d").clearRect(0,0,G.width,G.height),V(G,Y)}else M=document.createElement("canvas"),M.width=Y.size,M.height=Y.size,Y=V(M,Y),G.appendChild(Y)}})(function(){function l(G){var Y=Q.s(G);return{S:function(){return 4},b:function(){return Y.length},write:function(P){for(var X=0;X<Y.length;X+=1)P.put(Y[X],8)}}}function h(){var G=[],Y=0,P={B:function(){return G},c:function(X){return(G[Math.floor(X/8)]>>>7-X%8&1)==1},put:function(X,z){for(var j=0;j<z;j+=1)P.m((X>>>z-j-1&1)==1)},f:function(){return Y},m:function(X){var z=Math.floor(Y/8);G.length<=z&&G.push(0),X&&(G[z]|=128>>>Y%8),Y+=1}};return P}function Q(G,Y){function P(J,E){for(var N=-1;7>=N;N+=1)if(!(-1>=J+N||I<=J+N))for(var F=-1;7>=F;F+=1)-1>=E+F||I<=E+F||(j[J+N][E+F]=0<=N&&6>=N&&(F==0||F==6)||0<=F&&6>=F&&(N==0||N==6)||2<=N&&4>=N&&2<=F&&4>=F?!0:!1)}function X(J,E){for(var N=I=4*G+17,F=Array(N),D=0;D<N;D+=1){F[D]=Array(N);for(var w=0;w<N;w+=1)F[D][w]=null}j=F,P(0,0),P(I-7,0),P(0,I-7),N=V.G(G);for(F=0;F<N.length;F+=1)for(D=0;D<N.length;D+=1){w=N[F];var T=N[D];if(j[w][T]==null)for(var O=-2;2>=O;O+=1)for(var U=-2;2>=U;U+=1)j[w+O][T+U]=O==-2||O==2||U==-2||U==2||O==0&&U==0}for(N=8;N<I-8;N+=1)j[N][6]==null&&(j[N][6]=N%2==0);for(N=8;N<I-8;N+=1)j[6][N]==null&&(j[6][N]=N%2==0);N=V.w(z<<3|E);for(F=0;15>F;F+=1)D=!J&&(N>>F&1)==1,j[6>F?F:8>F?F+1:I-15+F][8]=D,j[8][8>F?I-F-1:9>F?15-F:14-F]=D;if(j[I-8][8]=!J,7<=G){N=V.A(G);for(F=0;18>F;F+=1)D=!J&&(N>>F&1)==1,j[Math.floor(F/3)][F%3+I-8-3]=D;for(F=0;18>F;F+=1)D=!J&&(N>>F&1)==1,j[F%3+I-8-3][Math.floor(F/3)]=D}if($==null){J=M.I(G,z),N=h();for(F=0;F<C.length;F+=1)D=C[F],N.put(4,4),N.put(D.b(),V.f(4,G)),D.write(N);for(F=D=0;F<J.length;F+=1)D+=J[F].j;if(N.f()>8*D)throw Error("code length overflow. ("+N.f()+">"+8*D+")");for(N.f()+4<=8*D&&N.put(0,4);N.f()%8!=0;)N.m(!1);for(;!(N.f()>=8*D);){if(N.put(236,8),N.f()>=8*D)break;N.put(17,8)}var m=0;D=F=0,w=Array(J.length),T=Array(J.length);for(O=0;O<J.length;O+=1){var a=J[O].j,r=J[O].o-a;F=Math.max(F,a),D=Math.max(D,r),w[O]=Array(a);for(U=0;U<w[O].length;U+=1)w[O][U]=255&N.B()[U+m];m+=a,U=V.C(r),a=K(w[O],U.b()-1).l(U),T[O]=Array(U.b()-1);for(U=0;U<T[O].length;U+=1)r=U+a.b()-T[O].length,T[O][U]=0<=r?a.c(r):0}for(U=N=0;U<J.length;U+=1)N+=J[U].o;N=Array(N);for(U=m=0;U<F;U+=1)for(O=0;O<J.length;O+=1)U<w[O].length&&(N[m]=w[O][U],m+=1);for(U=0;U<D;U+=1)for(O=0;O<J.length;O+=1)U<T[O].length&&(N[m]=T[O][U],m+=1);$=N}J=$,N=-1,F=I-1,D=7,w=0,E=V.F(E);for(T=I-1;0<T;T-=2)for(T==6&&--T;;){for(O=0;2>O;O+=1)j[F][T-O]==null&&(U=!1,w<J.length&&(U=(J[w]>>>D&1)==1),E(F,T-O)&&(U=!U),j[F][T-O]=U,--D,D==-1&&(w+=1,D=7));if(F+=N,0>F||I<=F){F-=N,N=-N;break}}}var z=Z[Y],j=null,I=0,$=null,C=[],L={u:function(J){J=l(J),C.push(J),$=null},a:function(J,E){if(0>J||I<=J||0>E||I<=E)throw Error(J+","+E);return j[J][E]},h:function(){return I},J:function(){for(var J=0,E=0,N=0;8>N;N+=1){X(!0,N);var F=V.D(L);if(N==0||J>F)J=F,E=N}X(!1,E)}};return L}function K(G,Y){if(typeof G.length>"u")throw Error(G.length+"/"+Y);var P=function(){for(var z=0;z<G.length&&G[z]==0;)z+=1;for(var j=Array(G.length-z+Y),I=0;I<G.length-z;I+=1)j[I]=G[I+z];return j}(),X={c:function(z){return P[z]},b:function(){return P.length},multiply:function(z){for(var j=Array(X.b()+z.b()-1),I=0;I<X.b();I+=1)for(var $=0;$<z.b();$+=1)j[I+$]^=W.i(W.g(X.c(I))+W.g(z.c($)));return K(j,0)},l:function(z){if(0>X.b()-z.b())return X;for(var j=W.g(X.c(0))-W.g(z.c(0)),I=Array(X.b()),$=0;$<X.b();$+=1)I[$]=X.c($);for($=0;$<z.b();$+=1)I[$]^=W.i(W.g(z.c($))+j);return K(I,0).l(z)}};return X}Q.s=function(G){for(var Y=[],P=0;P<G.length;P++){var X=G.charCodeAt(P);128>X?Y.push(X):2048>X?Y.push(192|X>>6,128|X&63):55296>X||57344<=X?Y.push(224|X>>12,128|X>>6&63,128|X&63):(P++,X=65536+((X&1023)<<10|G.charCodeAt(P)&1023),Y.push(240|X>>18,128|X>>12&63,128|X>>6&63,128|X&63))}return Y};var Z={L:1,M:0,Q:3,H:2},V=function(){function G(X){for(var z=0;X!=0;)z+=1,X>>>=1;return z}var Y=[[],[6,18],[6,22],[6,26],[6,30],[6,34],[6,22,38],[6,24,42],[6,26,46],[6,28,50],[6,30,54],[6,32,58],[6,34,62],[6,26,46,66],[6,26,48,70],[6,26,50,74],[6,30,54,78],[6,30,56,82],[6,30,58,86],[6,34,62,90],[6,28,50,72,94],[6,26,50,74,98],[6,30,54,78,102],[6,28,54,80,106],[6,32,58,84,110],[6,30,58,86,114],[6,34,62,90,118],[6,26,50,74,98,122],[6,30,54,78,102,126],[6,26,52,78,104,130],[6,30,56,82,108,134],[6,34,60,86,112,138],[6,30,58,86,114,142],[6,34,62,90,118,146],[6,30,54,78,102,126,150],[6,24,50,76,102,128,154],[6,28,54,80,106,132,158],[6,32,58,84,110,136,162],[6,26,54,82,110,138,166],[6,30,58,86,114,142,170]],P={w:function(X){for(var z=X<<10;0<=G(z)-G(1335);)z^=1335<<G(z)-G(1335);return(X<<10|z)^21522},A:function(X){for(var z=X<<12;0<=G(z)-G(7973);)z^=7973<<G(z)-G(7973);return X<<12|z},G:function(X){return Y[X-1]},F:function(X){switch(X){case 0:return function(z,j){return(z+j)%2==0};case 1:return function(z){return z%2==0};case 2:return function(z,j){return j%3==0};case 3:return function(z,j){return(z+j)%3==0};case 4:return function(z,j){return(Math.floor(z/2)+Math.floor(j/3))%2==0};case 5:return function(z,j){return z*j%2+z*j%3==0};case 6:return function(z,j){return(z*j%2+z*j%3)%2==0};case 7:return function(z,j){return(z*j%3+(z+j)%2)%2==0};default:throw Error("bad maskPattern:"+X)}},C:function(X){for(var z=K([1],0),j=0;j<X;j+=1)z=z.multiply(K([1,W.i(j)],0));return z},f:function(X,z){if(X!=4||1>z||40<z)throw Error("mode: "+X+"; type: "+z);return 10>z?8:16},D:function(X){for(var z=X.h(),j=0,I=0;I<z;I+=1)for(var $=0;$<z;$+=1){for(var C=0,L=X.a(I,$),J=-1;1>=J;J+=1)if(!(0>I+J||z<=I+J))for(var E=-1;1>=E;E+=1)0>$+E||z<=$+E||(J!=0||E!=0)&&L==X.a(I+J,$+E)&&(C+=1);5<C&&(j+=3+C-5)}for(I=0;I<z-1;I+=1)for($=0;$<z-1;$+=1)if(C=0,X.a(I,$)&&(C+=1),X.a(I+1,$)&&(C+=1),X.a(I,$+1)&&(C+=1),X.a(I+1,$+1)&&(C+=1),C==0||C==4)j+=3;for(I=0;I<z;I+=1)for($=0;$<z-6;$+=1)X.a(I,$)&&!X.a(I,$+1)&&X.a(I,$+2)&&X.a(I,$+3)&&X.a(I,$+4)&&!X.a(I,$+5)&&X.a(I,$+6)&&(j+=40);for($=0;$<z;$+=1)for(I=0;I<z-6;I+=1)X.a(I,$)&&!X.a(I+1,$)&&X.a(I+2,$)&&X.a(I+3,$)&&X.a(I+4,$)&&!X.a(I+5,$)&&X.a(I+6,$)&&(j+=40);for($=C=0;$<z;$+=1)for(I=0;I<z;I+=1)X.a(I,$)&&(C+=1);return j+=Math.abs(100*C/z/z-50)/5*10}};return P}(),W=function(){for(var G=Array(256),Y=Array(256),P=0;8>P;P+=1)G[P]=1<<P;for(P=8;256>P;P+=1)G[P]=G[P-4]^G[P-5]^G[P-6]^G[P-8];for(P=0;255>P;P+=1)Y[G[P]]=P;return{g:function(X){if(1>X)throw Error("glog("+X+")");return Y[X]},i:function(X){for(;0>X;)X+=255;for(;256<=X;)X-=255;return G[X]}}}(),M=function(){function G(X,z){switch(z){case Z.L:return Y[4*(X-1)];case Z.M:return Y[4*(X-1)+1];case Z.Q:return Y[4*(X-1)+2];case Z.H:return Y[4*(X-1)+3]}}var Y=[[1,26,19],[1,26,16],[1,26,13],[1,26,9],[1,44,34],[1,44,28],[1,44,22],[1,44,16],[1,70,55],[1,70,44],[2,35,17],[2,35,13],[1,100,80],[2,50,32],[2,50,24],[4,25,9],[1,134,108],[2,67,43],[2,33,15,2,34,16],[2,33,11,2,34,12],[2,86,68],[4,43,27],[4,43,19],[4,43,15],[2,98,78],[4,49,31],[2,32,14,4,33,15],[4,39,13,1,40,14],[2,121,97],[2,60,38,2,61,39],[4,40,18,2,41,19],[4,40,14,2,41,15],[2,146,116],[3,58,36,2,59,37],[4,36,16,4,37,17],[4,36,12,4,37,13],[2,86,68,2,87,69],[4,69,43,1,70,44],[6,43,19,2,44,20],[6,43,15,2,44,16],[4,101,81],[1,80,50,4,81,51],[4,50,22,4,51,23],[3,36,12,8,37,13],[2,116,92,2,117,93],[6,58,36,2,59,37],[4,46,20,6,47,21],[7,42,14,4,43,15],[4,133,107],[8,59,37,1,60,38],[8,44,20,4,45,21],[12,33,11,4,34,12],[3,145,115,1,146,116],[4,64,40,5,65,41],[11,36,16,5,37,17],[11,36,12,5,37,13],[5,109,87,1,110,88],[5,65,41,5,66,42],[5,54,24,7,55,25],[11,36,12,7,37,13],[5,122,98,1,123,99],[7,73,45,3,74,46],[15,43,19,2,44,20],[3,45,15,13,46,16],[1,135,107,5,136,108],[10,74,46,1,75,47],[1,50,22,15,51,23],[2,42,14,17,43,15],[5,150,120,1,151,121],[9,69,43,4,70,44],[17,50,22,1,51,23],[2,42,14,19,43,15],[3,141,113,4,142,114],[3,70,44,11,71,45],[17,47,21,4,48,22],[9,39,13,16,40,14],[3,135,107,5,136,108],[3,67,41,13,68,42],[15,54,24,5,55,25],[15,43,15,10,44,16],[4,144,116,4,145,117],[17,68,42],[17,50,22,6,51,23],[19,46,16,6,47,17],[2,139,111,7,140,112],[17,74,46],[7,54,24,16,55,25],[34,37,13],[4,151,121,5,152,122],[4,75,47,14,76,48],[11,54,24,14,55,25],[16,45,15,14,46,16],[6,147,117,4,148,118],[6,73,45,14,74,46],[11,54,24,16,55,25],[30,46,16,2,47,17],[8,132,106,4,133,107],[8,75,47,13,76,48],[7,54,24,22,55,25],[22,45,15,13,46,16],[10,142,114,2,143,115],[19,74,46,4,75,47],[28,50,22,6,51,23],[33,46,16,4,47,17],[8,152,122,4,153,123],[22,73,45,3,74,46],[8,53,23,26,54,24],[12,45,15,28,46,16],[3,147,117,10,148,118],[3,73,45,23,74,46],[4,54,24,31,55,25],[11,45,15,31,46,16],[7,146,116,7,147,117],[21,73,45,7,74,46],[1,53,23,37,54,24],[19,45,15,26,46,16],[5,145,115,10,146,116],[19,75,47,10,76,48],[15,54,24,25,55,25],[23,45,15,25,46,16],[13,145,115,3,146,116],[2,74,46,29,75,47],[42,54,24,1,55,25],[23,45,15,28,46,16],[17,145,115],[10,74,46,23,75,47],[10,54,24,35,55,25],[19,45,15,35,46,16],[17,145,115,1,146,116],[14,74,46,21,75,47],[29,54,24,19,55,25],[11,45,15,46,46,16],[13,145,115,6,146,116],[14,74,46,23,75,47],[44,54,24,7,55,25],[59,46,16,1,47,17],[12,151,121,7,152,122],[12,75,47,26,76,48],[39,54,24,14,55,25],[22,45,15,41,46,16],[6,151,121,14,152,122],[6,75,47,34,76,48],[46,54,24,10,55,25],[2,45,15,64,46,16],[17,152,122,4,153,123],[29,74,46,14,75,47],[49,54,24,10,55,25],[24,45,15,46,46,16],[4,152,122,18,153,123],[13,74,46,32,75,47],[48,54,24,14,55,25],[42,45,15,32,46,16],[20,147,117,4,148,118],[40,75,47,7,76,48],[43,54,24,22,55,25],[10,45,15,67,46,16],[19,148,118,6,149,119],[18,75,47,31,76,48],[34,54,24,34,55,25],[20,45,15,61,46,16]],P={I:function(X,z){var j=G(X,z);if(typeof j>"u")throw Error("bad rs block @ typeNumber:"+X+"/errorCorrectLevel:"+z);X=j.length/3,z=[];for(var I=0;I<X;I+=1)for(var $=j[3*I],C=j[3*I+1],L=j[3*I+2],J=0;J<$;J+=1){var E=L,N={};N.o=C,N.j=E,z.push(N)}return z}};return P}();return Q}());var pK=QrCreator;var KZ="#260133",ZZ="#1F2348";function nK(l,h,Q){if(!l||typeof getComputedStyle!=="function")return Q;return getComputedStyle(l).getPropertyValue(h).trim()||Q}function sK(l,h,Q){let K=document.createElement("canvas");K.className="nq-cc-qr-canvas";let Z=Math.min(3,Math.max(1,Math.round(globalThis.devicePixelRatio||1)));return pK.render({text:l,radius:0.5,ecLevel:"M",fill:{type:"radial-gradient",position:[1,1,0,1,1,Math.SQRT2],colorStops:[[0,nK(Q??null,"--nq-cc-qr-from",KZ)],[1,nK(Q??null,"--nq-cc-qr-to",ZZ)]]},background:null,size:h*Z},K),K.style.width=`${h}px`,K.style.height=`${h}px`,K}function jh(l){return l.replace(/^\s*(?:nimiq:)?/i,"").replace(/[^0-9A-Za-z]/g,"").toUpperCase().slice(0,36)}function dl(l){let Q=jh(l).match(/.{1,4}/g)??[],K=[];for(let Z=0;Z<Q.length;Z+=3)K.push(Q.slice(Z,Z+3).join(" "));return K.join(`
`)}function zZ(l,h){if(h<=0)return 0;let Q=0;for(let K=0;K<l.length;K+=1)if(/[0-9A-Z]/.test(l[K])){if(Q+=1,Q===h)return K+1}return l.length}function XZ(l,h){return jh(l.slice(0,h)).length}function aK(l){let h=l.selectionStart??l.value.length,Q=XZ(l.value,h),K=dl(l.value);if(K!==l.value)l.value=K;let Z=zZ(K,Q);l.setSelectionRange(Z,Z)}var GZ=/^NQ[0-9]{2}[0-9A-HJ-NP-VXY]{32}$/;function K0(l,h,Q,K=8){let Z=K-l;if(Z<=0)return 0;let V=Q-K-h,W=Math.min(Z,Math.max(0,V));return W>0?-Math.round(W):0}function Z0(l){let h=l.replace(/\s+/g,"");if(!h)return{cells:[],columns:1};if(GZ.test(h.toUpperCase()))return{cells:h.match(/.{4}/g)??[h],columns:3};let Q=h.length,K=Math.floor(Q/3),Z=Q%3,V=[],W=0;for(let M=0;M<3;M+=1){let G=K+(M<Z?1:0);if(G===0)continue;V.push(h.slice(W,W+G)),W+=G}return{cells:V,columns:1}}function YZ(l){return typeof TextEncoder==="function"?new TextEncoder().encode(l).length:encodeURIComponent(l).replace(/%[0-9A-F]{2}/g,"x").length}var rK="nimiq-shell-corner-control-style",oK="nq-shell:fiat",z0="nq-shell:label:";function kZ(l){try{return localStorage.getItem(z0+l.replace(/\s+/g,""))}catch{return null}}function VZ(l,h){try{localStorage.setItem(z0+l.replace(/\s+/g,""),h)}catch{}}var WZ={en:"English",es:"Español",de:"Deutsch",fr:"Français",pt:"Português",hi:"हिन्दी",zh:"中文",tr:"Türkçe",ko:"한국어",vi:"Tiếng Việt",ha:"Hausa",tl:"Filipino",id:"Bahasa Indonesia"},tK={AED:"ae",ARS:"ar",AUD:"au",BRL:"br",CAD:"ca",CHF:"ch",CLP:"cl",CNY:"cn",CRC:"cr",CZK:"cz",DKK:"dk",EUR:"eu",GBP:"gb",GMD:"gm",GTQ:"gt",HKD:"hk",HUF:"hu",IDR:"id",ILS:"il",INR:"in",JPY:"jp",KRW:"kr",MXN:"mx",MYR:"my",NGN:"ng",NOK:"no",NZD:"nz",PHP:"ph",PKR:"pk",PLN:"pl",RUB:"ru",SEK:"se",SGD:"sg",THB:"th",TRY:"tr",TWD:"tw",UAH:"ua",USD:"us",VND:"vn",ZAR:"za"};function MZ(){if(typeof document>"u"||document.getElementById(rK))return;let l=document.createElement("style");l.id=rK,l.textContent=`
...
`,document.head.appendChild(l)}var jZ='<svg class="nq-cc-cross" viewBox="0 0 10 10" aria-hidden="true"><path d="M1 1l8 8M9 1l-8 8" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg>',$Z='<svg class="nq-cc-chevron" viewBox="0 0 6 10" aria-hidden="true"><path d="M5 1L1 5l4 4" fill="none" stroke="currentColor" stroke-width="1.15" stroke-linecap="round" stroke-linejoin="round"/></svg>',$h='<svg class="nq-cc-caret" viewBox="0 0 10 6" aria-hidden="true"><path d="M1 1l4 4 4-4" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>',MQ='<svg viewBox="0 -4 64 64" aria-hidden="true"><path opacity=".25" d="M62.3 25.4L49.2 2.6A5.3 5.3 0 0 0 44.6 0H18.4c-1.9 0-3.6 1-4.6 2.6L.7 25.4c-1 1.6-1 3.6 0 5.2l13.1 22.8c1 1.6 2.7 2.6 4.6 2.6h26.2c1.9 0 3.6-1 4.6-2.6l13-22.8c1-1.6 1-3.6.1-5.2z" fill="currentColor"/></svg>',AZ='<svg width="18" height="18" viewBox="0 0 20 20" fill="none" aria-hidden="true" style="flex:none;opacity:.85"><rect x="2" y="5" width="16" height="12" rx="2" stroke="currentColor" stroke-width="1.5"/><path d="M2 9h16" stroke="currentColor" stroke-width="1.5"/><circle cx="6" cy="13" r="1" fill="currentColor"/></svg>',eK='<svg width="16" height="12" viewBox="0 0 16 12" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="%CLS%"><path d="M10,1l5,5l-5,5" stroke="currentColor" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><line x1="14" y1="6" x2="1" y2="6" stroke="currentColor" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',IZ='<svg class="nq-cc-book-glyph" viewBox="0 0 10 12" aria-hidden="true"><path fill="currentColor" fill-rule="evenodd" d="M8.75 1.776c0 .117.083.22.201.244a1 1 0 01.798.98v8a1 1 0 01-.999 1H1.501A1.5 1.5 0 010 10.499V1.5A1.5 1.5 0 011.501 0h6.25a1 1 0 011 1zM7.501 9.583a2.505 2.505 0 00-2.505-2.334A2.51 2.51 0 002.5 9.473a.25.25 0 00.25.276h4.592a.157.157 0 00.157-.166zM3.49 5.248a1.506 1.506 0 110 .003zM1.499 1.002a.5.5 0 000 1.001h6.125a.124.124 0 00.125-.125v-.754a.125.125 0 00-.123-.122h-6.13z" clip-rule="evenodd"/></svg>',JZ='<svg class="nq-cc-qr-glyph" viewBox="0 0 13 13" aria-hidden="true"><g fill="none"><path fill="currentColor" d="M5.796 1.902a.304.304 0 00-.304-.305H1.938a.305.305 0 00-.304.305v3.553c0 .168.136.305.304.305h3.553a.305.305 0 00.305-.305zm-.609 3.249H2.243V2.206h2.944z"/><path stroke="currentColor" stroke-width=".091" d="M5.796 1.902a.304.304 0 00-.304-.305H1.938a.305.305 0 00-.304.305v3.553c0 .168.136.305.304.305h3.553a.305.305 0 00.305-.305zm0 0h-.049m-.56 3.249H2.243V2.206h2.944z"/><path fill="currentColor" stroke="currentColor" stroke-width=".091" d="M3.067 4.632h1.296a.305.305 0 00.305-.305V3.03a.305.305 0 00-.305-.304H3.067a.305.305 0 00-.305.304v1.297c0 .168.137.305.305.305Zm.304-1.297h.688v.688H3.37zM7.509 5.76h3.553a.304.304 0 00.304-.305V1.902a.304.304 0 00-.304-.305H7.509a.305.305 0 00-.305.305v3.553c0 .168.136.305.305.305Zm.304-3.554h2.944v2.945H7.813z"/><path fill="currentColor" stroke="currentColor" stroke-width=".091" d="M9.933 2.726H8.636a.304.304 0 00-.304.304v1.297c0 .168.136.305.304.305h1.297a.305.305 0 00.305-.305V3.03a.305.305 0 00-.305-.304Zm-.305 1.297h-.687v-.688h.687z"/><path fill="currentColor" d="M5.796 7.472a.304.304 0 00-.304-.304H1.938a.304.304 0 00-.304.304v3.553c0 .169.136.305.304.305h3.553a.305.305 0 00.305-.305zm-.609 3.249H2.243V7.777h2.944z"/><path stroke="currentColor" stroke-width=".091" d="M5.796 7.472a.304.304 0 00-.304-.304H1.938a.304.304 0 00-.304.304v3.553c0 .169.136.305.304.305h3.553a.305.305 0 00.305-.305zm0 0h-.049m-.56 3.249H2.243V7.777h2.944z"/><path fill="currentColor" stroke="currentColor" stroke-width=".091" d="M3.067 10.202h1.296a.305.305 0 00.305-.305V8.6a.304.304 0 00-.305-.304H3.067a.304.304 0 00-.305.304v1.297c0 .168.137.305.305.305Zm.304-1.297h.688v.688H3.37zm7.995 2.12V7.472a.304.304 0 00-.304-.304H7.509a.304.304 0 00-.305.304v3.553c0 .169.136.305.305.305h3.553a.305.305 0 00.304-.305Zm-.609-.304H7.813V7.777h2.944z"/><path fill="currentColor" stroke="currentColor" stroke-width=".091" d="M8.637 10.202h1.297a.304.304 0 00.304-.305V8.6a.304.304 0 00-.304-.304H8.637a.304.304 0 00-.304.304v1.297c0 .168.136.305.304.305Zm.305-1.297h.687v.688h-.687zM.353 3.323a.305.305 0 00.305-.304V.658h2.36a.305.305 0 000-.61H.353A.305.305 0 00.05.354V3.02c0 .168.136.304.304.304ZM12.647.049H9.982a.304.304 0 100 .609h2.36v2.361a.305.305 0 10.61 0V.353a.304.304 0 00-.305-.304Zm-.001 9.556a.304.304 0 00-.304.304v2.36h-2.36a.304.304 0 100 .61h2.664a.304.304 0 00.305-.305V9.91a.304.304 0 00-.305-.304ZM3.018 12.27H.658V9.91a.304.304 0 10-.61 0v2.664c0 .169.137.305.305.305h2.665a.305.305 0 100-.61Z"/></g></svg>',l0='<svg class="nq-cc-scan-glyph" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><g fill="currentColor"><path d="M1.21 7.06c.67 0 1.21-.54 1.21-1.21l-.04-3.12a.3.3 0 0 1 .3-.3H5.7a1.21 1.21 0 1 0 0-2.43H2.37A2.4 2.4 0 0 0 0 2.42v3.43c0 .67.54 1.21 1.21 1.21zM5.69 37.58H2.73a.3.3 0 0 1-.3-.3v-3.13a1.21 1.21 0 1 0-2.43 0v3.43A2.4 2.4 0 0 0 2.37 40H5.7a1.21 1.21 0 0 0 0-2.42zM38.79 32.94c-.67 0-1.21.54-1.21 1.21l.04 3.12a.3.3 0 0 1-.3.3H34.3a1.21 1.21 0 1 0 0 2.43h3.32A2.4 2.4 0 0 0 40 37.58v-3.43c0-.67-.54-1.21-1.21-1.21zM37.63 0H34.3a1.21 1.21 0 1 0 0 2.42h2.96c.17 0 .3.14.3.3v3.13a1.21 1.21 0 0 0 2.43 0V2.42A2.4 2.4 0 0 0 37.63 0z"/><path fill-rule="evenodd" clip-rule="evenodd" d="M13.94 15.15H6.67c-.67 0-1.22-.54-1.22-1.21V6.67c0-.67.55-1.21 1.22-1.21h7.27c.67 0 1.21.54 1.21 1.2v7.28c0 .67-.54 1.21-1.21 1.21zM8.18 7.88a.3.3 0 0 0-.3.3v4.24c0 .17.13.3.3.3h4.24a.3.3 0 0 0 .3-.3V8.18a.3.3 0 0 0-.3-.3H8.18zM6.67 24.85h7.27c.67 0 1.21.54 1.21 1.21v7.27c0 .67-.54 1.22-1.21 1.22H6.67c-.67 0-1.22-.55-1.22-1.22v-7.27c0-.67.55-1.21 1.22-1.21zm5.75 7.27a.3.3 0 0 0 .3-.3v-4.24a.3.3 0 0 0-.3-.3H8.18a.3.3 0 0 0-.3.3v4.24c0 .17.13.3.3.3h4.24zM26.06 5.45h7.27c.67 0 1.21.55 1.21 1.22v7.27c0 .67-.54 1.21-1.2 1.21h-7.28c-.67 0-1.21-.54-1.21-1.21V6.67c0-.67.54-1.22 1.21-1.22zm5.76 7.28a.3.3 0 0 0 .3-.3V8.17a.3.3 0 0 0-.3-.3h-4.24a.3.3 0 0 0-.3.3v4.24c0 .17.13.3.3.3h4.24z"/><path d="M17.58 10.6h1.2a.9.9 0 1 0 0-1.81.3.3 0 0 1-.3-.3V6.66a.9.9 0 1 0-1.81 0V9.7c0 .5.4.9.9.9zM21.21 7.58c.17 0 .3.13.3.3v6.66a.9.9 0 1 0 1.82 0V6.67c0-.5-.4-.91-.9-.91H21.2a.9.9 0 1 0 0 1.82zM12.42 18.18c0 .5.41.91.91.91h4.25c.5 0 .9-.4.9-.9v-4.86a.9.9 0 1 0-1.81 0v3.64a.3.3 0 0 1-.3.3h-3.04c-.5 0-.9.4-.9.91z"/><path d="M9.09 17.27c-.5 0-.9.4-.9.91v3.03a.3.3 0 0 1-.31.3H6.67a.9.9 0 1 0 0 1.82h15.75c.5 0 .91-.4.91-.9v-3.64a.9.9 0 0 0-1.82 0v2.42a.3.3 0 0 1-.3.3h-10.9a.3.3 0 0 1-.31-.3v-3.03c0-.5-.4-.9-.91-.9zM22.12 26.06c0-.5-.4-.9-.9-.9h-3.64c-.5 0-.91.4-.91.9v4.85a.9.9 0 1 0 1.81 0v-3.64c0-.16.14-.3.3-.3h2.43c.5 0 .91-.4.91-.9zM33.33 32.42h-10.3a.3.3 0 0 1-.3-.3V29.7a.9.9 0 1 0-1.82 0v3.63c0 .5.4.91.9.91h11.52a.9.9 0 0 0 0-1.82z"/><path fill-rule="evenodd" clip-rule="evenodd" d="M29.1 30h-3.65a.9.9 0 0 1-.9-.91v-3.64c0-.5.4-.9.9-.9h3.64c.5 0 .91.4.91.9v3.64c0 .5-.4.91-.9.91zm-2.43-3.64a.3.3 0 0 0-.3.3v1.22c0 .17.13.3.3.3h1.2a.3.3 0 0 0 .31-.3v-1.21a.3.3 0 0 0-.3-.3h-1.21z"/><path d="M32.73 20.9c-.5 0-.91.42-.91.92v7.88a.9.9 0 0 0 1.82 0v-7.88c0-.5-.41-.91-.91-.91zM33.64 17.58c0-.5-.41-.91-.91-.91h-6.67c-.5 0-.9.4-.9.9v3.64a.9.9 0 0 0 1.8 0V18.8c0-.17.15-.3.31-.3h5.46c.5 0 .9-.41.9-.91z"/></g></svg>',PZ='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 18 17" aria-hidden="true"><g fill="none" stroke="currentColor" stroke-width="1.28" stroke-linecap="round" stroke-linejoin="round"><path d="M16.603 6.779l-2.987-5.306a1.37 1.37 0 00-1.189-.702H5.57c-.489 0-.941.267-1.186.703L1.396 6.779"/><path d="M1.396 9.752l2.988 5.304a1.36 1.36 0 001.186.703h6.858a1.36 1.36 0 001.186-.703l2.988-5.304"/><path d="M3.24 5.773L1.396 6.779L1.3 4.681M14.758 10.757L16.602 9.752L16.697 11.85"/></g></svg>',NZ='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" aria-hidden="true"><g fill="none" stroke="currentColor" stroke-linecap="round" stroke-width="2.5px" stroke-linejoin="round"><path d="M40.25,23.25v-.5a6.5,6.5,0,0,0-6.5-6.5h-3.5a6.5,6.5,0,0,0-6.5,6.5v6.5a6.5,6.5,0,0,0,6.5,6.5h2"/><path d="M23.75,40.75v.5a6.5,6.5,0,0,0,6.5,6.5h3.5a6.5,6.5,0,0,0,6.5-6.5v-6.5a6.5,6.5,0,0,0-6.5-6.5h-2"/><line x1="32" y1="11.25" x2="32" y2="15.25"/><line x1="32" y1="48.75" x2="32" y2="52.75"/></g></svg>',CZ=0;function _Z(){let l=`nq-cc-hex-${CZ+=1}`;return`<svg class="nq-cc-hexlogo" viewBox="0 0 20 18" aria-hidden="true"><g fill="none"><path fill="url(#${l})" d="M19.964 8.156 15.758.844A1.69 1.69 0 0014.299 0H5.887c-.6 0-1.156.32-1.456.844L.225 8.156c-.3.523-.3 1.165 0 1.688l4.206 7.312c.3.523.856.844 1.456.844h8.412c.6 0 1.156-.32 1.456-.844l4.206-7.312a1.69 1.69 0 00.003-1.688"/><defs><radialGradient id="${l}" cx="0" cy="0" r="1" gradientTransform="matrix(20.1956 0 0 20.2552 15.188 17.766)" gradientUnits="userSpaceOnUse"><stop stop-color="#ec991c"/><stop offset="1" stop-color="#e9b213"/></radialGradient></defs></g></svg>`}function h0(l,h){let Q=l.address?kZ(l.address):null;if(Q)return Q;if(l.label)return l.label;let K=l.address?.trim()??"";if(!K)return h;return`${K.slice(0,7)}…${K.slice(-4)}`}function A(l,h,Q){let K=document.createElement(l);if(h)K.className=h;if(Q)Q.appendChild(K);return K}function Q0(l,h){let Q=document.createElement("div");Q.className="nq-cc-grid-more",Q.setAttribute("aria-hidden","true"),Q.innerHTML='<svg viewBox="0 0 12 8" fill="none" aria-hidden="true"><path d="M1 1.5 6 6.5l5-5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',l.appendChild(Q);let K=()=>{let Z=h.scrollHeight-h.clientHeight>2;l.toggleAttribute("data-no-scroll",!Z),l.toggleAttribute("data-at-end",Z&&h.scrollTop+h.clientHeight>=h.scrollHeight-2)};if(h.addEventListener("scroll",K,{passive:!0}),typeof ResizeObserver<"u")new ResizeObserver(K).observe(h);K()}function EZ(){if(typeof window>"u"||!window.location)return;let l=GQ(window.location.hostname);return l?{bot:{repo:l}}:void 0}function X0(l,h){let{wallet:Q,i18n:K}=h,Z=h.languages??bl,V=h.receive!==!1,W=!!h.assets,M=h.getBalanceLuna??(W||h.balance===!1||h.network==="test"?void 0:ah(h.balance&&h.balance.rpc?{rpc:h.balance.rpc}:{})),G=!!M||W,Y=!!h.fiat&&h.fiat.currencies.length>0;if(h.injectStyles!==!1)MZ();let P=h.fiat?.default??"USD";try{let k=localStorage.getItem(oK);if(k&&h.fiat?.currencies.includes(k))P=k}catch{}if(h.fiat&&!h.fiat.currencies.includes(P))P=h.fiat.currencies[0];let X=A("div","nq-cc");if(X.dataset.mode=!Q||Q.mode==="miniapp"?"miniapp":"hub",!Q)X.dataset.face="lang";if(h.network==="test")X.dataset.testnet="";if(h.theme)Ul(X,h.theme);l.appendChild(X);let z=(k)=>Z.find((_)=>_.id===k)??Z[0],j=(k)=>WZ[k.id]??k.name,I=A("button","nq-cc-face",X);I.type="button",I.setAttribute("aria-haspopup","menu"),I.setAttribute("aria-expanded","false");let $=A("button","nq-cc-face-flag",X);$.type="button",$.setAttribute("aria-haspopup","menu"),$.setAttribute("aria-expanded","false");function C(){I.textContent="";let k=Q?.account??null;if(k){X.dataset.connected="";let _=A("span","nq-cc-face-icon",I);if(h.identicon)_.appendChild(h.identicon(k.address,28));else _.innerHTML=MQ;let H=A("span","nq-cc-face-label",I);H.textContent=h0(k,K.t("shell.account"))}else{delete X.dataset.connected,I.insertAdjacentHTML("beforeend",AZ);let _=A("span","nq-cc-face-label",I);_.textContent=K.t("shell.connectWallet")}I.insertAdjacentHTML("beforeend",$h)}function L(){$.textContent="";let k=z(K.getLanguage());$.setAttribute("aria-label",K.t("shell.language")),$.appendChild(s(k.flag,{size:24})),$.insertAdjacentHTML("beforeend",$h)}let J=A("div","nq-cc-menu",X);J.hidden=!0;let E=A("div","nq-cc-view-receive",J),N=A("div","nq-cc-view-qr",J),F=A("div","nq-cc-view-request",J),D=A("div","nq-cc-view-sendto",J),w=A("div","nq-cc-view-send",J),T=A("div","nq-cc-view-main",J),O=[];function U(k,_){O.push([k,_]),k.textContent=K.t(_)}let m=()=>{};function a(){for(let[k,_]of O)k.textContent=K.t(_);m()}let r=A("div","nq-cc-section nq-cc-when-out nq-cc-when-hub",T),c=A("button","nq-cc-connect",r);if(c.type="button",U(c,"shell.connectWallet"),c.addEventListener("click",async()=>{if(!Q)return;c.disabled=!0,c.textContent=K.t("shell.connecting");try{await Q.connect(),B(!1),c.textContent=K.t("shell.connectWallet")}catch{c.textContent=K.t("shell.retry")}finally{c.disabled=!1}}),h.onboard){let k=A("button","nq-cc-onboard",r);k.type="button",U(k,"shell.newToNimiq"),k.addEventListener("click",()=>h.onboard())}A("div","nq-cc-divider nq-cc-when-out nq-cc-when-hub",T);let hl=A("div","nq-cc-section nq-cc-wallet nq-cc-when-connected nq-cc-when-hub",T),ul=A("div","nq-cc-account",hl),Ah=A("span","nq-cc-identicon",ul),cl=A("button","nq-cc-name",ul);cl.type="button",cl.addEventListener("click",()=>G0(cl));let kl=A("div","nq-cc-balance",ul);kl.hidden=!0;let jQ=A("span","nq-cc-balance-nim",kl),Il=A("span","nq-cc-balance-fiat",kl),e=null;if(W)e=VQ(hl,{assets:h.assets,fiatTicker:()=>P,rate:Y?(k)=>h.fiat.rate(P,k):void 0,autoRefresh:!1,onSelect:V?(k)=>nQ(k):void 0});function G0(k){if(k.querySelector("input"))return;let _=k.textContent??"";k.textContent="";let H=document.createElement("input");H.className="nq-cc-name-input",H.value=_,H.maxLength=24,k.appendChild(H),H.focus(),H.select();let S=()=>{let y=H.value.trim()||_;if(k.textContent=y,y!==_){if(Q?.account)VZ(Q.account.address,y);if(C(),h.onRename)h.onRename(y)}};H.addEventListener("blur",S),H.addEventListener("keydown",(y)=>{if(y.key==="Enter")H.blur();if(y.key==="Escape")H.value=_,H.blur()})}{let k=A("div","nq-cc-actions",hl);if(V){let _=A("button","nq-cc-receive",k);_.type="button",_.insertAdjacentHTML("beforeend",eK.replace("%CLS%","nq-cc-arrow-down"));let H=A("span",void 0,_);U(H,"shell.receive"),_.addEventListener("click",()=>nQ())}{let _=A("button","nq-cc-send",k);_.type="button",_.insertAdjacentHTML("beforeend",eK.replace("%CLS%","nq-cc-arrow-up"));let H=A("span",void 0,_);U(H,"shell.send"),_.addEventListener("click",()=>{if(h.send)B(!1),h.send();else v0()})}if(h.scan){let _=A("button","nq-cc-scan",k);_.type="button",_.setAttribute("aria-label","Scan QR code"),_.insertAdjacentHTML("beforeend",l0),_.addEventListener("click",()=>{B(!1),h.scan()})}}if(A("div","nq-cc-divider nq-cc-when-connected nq-cc-when-hub",T),h.createCashlink){let k=A("button","nq-cc-row nq-cc-when-connected nq-cc-when-hub",T);k.type="button";let _=A("span","nq-cc-cashlink-slot",k);_.innerHTML=NZ;let H=A("span","nq-cc-strong",k);U(H,"shell.createCashlink"),k.addEventListener("click",()=>{B(!1),h.createCashlink()}),A("div","nq-cc-divider nq-cc-when-connected nq-cc-when-hub",T)}let $Q=A("div","nq-cc-section",T),Vl=A("button","nq-cc-acc",$Q);Vl.type="button",Vl.setAttribute("aria-expanded","false");let Y0=A("span","nq-cc-label",Vl);U(Y0,"shell.language");let AQ=A("span","nq-cc-acc-value",Vl),IQ=A("span",void 0,AQ),k0=A("span","nq-cc-strong",AQ);Vl.insertAdjacentHTML("beforeend",$h);let Ih=A("div","nq-cc-acc-body",$Q),JQ=A("div","nq-cc-grid-wrap",Ih),il=A("div","nq-cc-grid nq-cc-cols-2",JQ);Q0(JQ,il),il.setAttribute("role","listbox"),il.setAttribute("aria-label",K.t("shell.language"));let PQ=new Map;for(let k of Z){let _=A("button","nq-cc-card",il);_.type="button",_.setAttribute("role","option"),A("span","nq-cc-card-art",_).appendChild(s(k.flag,{size:26}));let S=A("span","nq-cc-card-name",_);S.textContent=j(k),_.addEventListener("click",()=>{K.setLanguage(k.id),window.setTimeout(()=>Sh(Vl,Ih),260)}),PQ.set(k.id,_)}function NQ(){let k=z(K.getLanguage());IQ.textContent="",IQ.appendChild(s(k.flag,{size:24})),k0.textContent=j(k);for(let[_,H]of PQ){let S=_===k.id;H.classList.toggle("nq-cc-current",S),H.setAttribute("aria-selected",String(S))}}let pl=null,Jh=null,CQ=new Map;if(Y){let k=G?" nq-cc-when-connected":"";A("div",`nq-cc-divider${k}`,T);let _=A("div",`nq-cc-section${k}`,T),H=A("button","nq-cc-acc",_);H.type="button",H.setAttribute("aria-expanded","false");let S=A("span","nq-cc-label",H);U(S,"shell.amountsIn");let y=A("span","nq-cc-acc-value",H);pl=A("span",void 0,y),Jh=A("span","nq-cc-strong",y),H.insertAdjacentHTML("beforeend",$h);let v=A("div","nq-cc-acc-body",_),g=A("div","nq-cc-grid-wrap",v),p=A("div","nq-cc-grid nq-cc-cols-3",g);Q0(g,p),p.setAttribute("role","listbox"),p.setAttribute("aria-label",K.t("shell.amountsIn"));for(let n of h.fiat.currencies){let Cl=A("button","nq-cc-card",p);Cl.type="button",Cl.setAttribute("role","option");let g0=A("span","nq-cc-card-art",Cl),hK=tK[n];if(hK)g0.appendChild(s(hK,{size:26}));let m0=A("span","nq-cc-card-ticker",Cl);m0.textContent=n,Cl.addEventListener("click",()=>{if(n===P){window.setTimeout(()=>Sh(H,v),260);return}P=n;try{localStorage.setItem(oK,n)}catch{}if(_Q(),G)yl(!0);Sl=null,h.fiat.onChange?.(n),window.setTimeout(()=>Sh(H,v),260)}),CQ.set(n,Cl)}sQ(H,v)}function _Q(){if(!pl||!Jh)return;pl.textContent="";let k=tK[P];if(k)pl.appendChild(s(k,{size:24}));Jh.textContent=P;for(let[_,H]of CQ){let S=_===P;H.classList.toggle("nq-cc-current",S),H.setAttribute("aria-selected",String(S))}}if(h.openInPay){A("div","nq-cc-divider nq-cc-when-hub",T);let k=A("button","nq-cc-row nq-cc-when-hub",T);k.type="button",k.insertAdjacentHTML("beforeend",_Z());let _=A("span","nq-cc-strong",k);U(_,"shell.openInPay"),k.addEventListener("click",()=>{B(!1);let H=h.openInPay;window.location.href=typeof H==="function"?H():H})}let Wl=h.reportBug===!1?void 0:h.reportBug??EZ();if(typeof Wl==="object"&&Wl.bot)KQ(Wl.bot.service??"https://bot.nimiq.tech");let EQ=!!Q&&h.switchAccount!==!1;if(EQ||Wl)A("div",`nq-cc-divider${Wl?"":" nq-cc-when-connected nq-cc-when-hub"}`,T);if(EQ){let k=A("button","nq-cc-row nq-cc-when-connected nq-cc-when-hub",T);k.type="button",A("span","nq-cc-cashlink-slot",k).insertAdjacentHTML("beforeend",PZ);let H=A("span","nq-cc-strong",k);U(H,"shell.switchAccount"),k.addEventListener("click",async()=>{B(!1);try{await Q.connect()}catch{}})}if(Wl){let k=A("button","nq-cc-row nq-cc-report",T);k.type="button",A("span","nq-cc-cashlink-slot",k).insertAdjacentHTML("beforeend",YQ);let H=A("span","nq-cc-strong",k);U(H,"shell.reportBug"),k.addEventListener("click",()=>{B(!1);let S=Wl;if(typeof S==="function")S();else kQ(document,K,h.theme?{...S,theme:S.theme??h.theme}:S)})}A("div","nq-cc-divider nq-cc-footer-divider",T);let FQ=A("div","nq-cc-footer",T),Ph=A("button","nq-cc-disconnect nq-cc-when-connected nq-cc-when-hub",FQ);Ph.type="button",U(Ph,"shell.disconnect"),Ph.addEventListener("click",()=>{Q?.disconnect(),B(!1)});let UQ=A("span","nq-cc-net-group",FQ),V0=A("span",void 0,UQ);U(V0,"shell.network");let W0=A("span","nq-cc-badge",UQ);W0.textContent="Testnet";function Hl(k,_,H){let S=A("div","nq-cc-view-head",k),y=A("button","nq-cc-back",S);y.type="button",y.setAttribute("aria-label",K.t("shell.back")),y.insertAdjacentHTML("beforeend",$Z);let v=A("span","nq-cc-view-title",S);U(v,_),y.addEventListener("click",H),HQ.push(y);let g=A("button","nq-cc-shut",S);return g.type="button",g.setAttribute("aria-label",K.t("shell.close")),A("span","nq-cc-shut-disc",g).insertAdjacentHTML("beforeend",jZ),g.addEventListener("click",()=>B(!1)),LQ.push(g),v}let HQ=[],LQ=[],xQ=[],DQ=[],M0=Hl(E,"shell.receive",()=>X.classList.remove("nq-cc-show-receive")),j0=A("p","nq-cc-view-sub",E);U(j0,"shell.receiveSub");let Ll=A("div","nq-cc-receive-body",E),nl=A("div","nq-cc-receive-hero",Ll);nl.hidden=!0;let Nh=A("div","nq-cc-qr",Ll),xl=A("span","nq-cc-copy-wrap",Ll),Ql=A("button","nq-cc-address",xl);Ql.type="button",Ql.title=K.t("shell.copyAddress");let OQ=A("span","nq-cc-copy-tooltip",xl);OQ.setAttribute("aria-hidden","true"),U(OQ,"shell.copied");let sl=A("div","nq-cc-receive-foot",Ll);sl.hidden=!0;let al=A("button","nq-cc-request-open",sl);al.type="button",U(al,"shell.requestLink"),al.addEventListener("click",()=>F0());let Dl=A("button","nq-cc-qr-open",sl);Dl.type="button",Dl.setAttribute("aria-label",K.t("shell.showQr")),Dl.insertAdjacentHTML("beforeend",JZ),Dl.addEventListener("click",()=>X.classList.add("nq-cc-show-qr")),DQ.push(Dl);let Ch=A("p","nq-cc-net-warn",Ll);Ch.hidden=!0;let $0=Hl(N,"shell.addressSheet",()=>X.classList.remove("nq-cc-show-qr")),_h=A("div","nq-cc-qr-body",N),A0=A("div","nq-cc-qr",_h),I0=A("p","nq-cc-qr-line",_h),J0=A("p","nq-cc-qr-scan",_h);function P0(k){let _=k.toUpperCase().match(/.{1,4}/g)??[k];if(_.length<=4)return _.join(" ");return`${_.slice(0,2).join(" ")} ••• ${_.slice(-2).join(" ")}`}let N0=Hl(F,"shell.requestTitle",()=>X.classList.remove("nq-cc-show-request")),Ol=A("div","nq-cc-request-body",F),C0=A("label","nq-cc-field-label",Ol);U(C0,"shell.amount");let TQ=A("div","nq-cc-amount-row",Ol),Jl=A("input","nq-cc-input",TQ);Jl.placeholder="0",Jl.inputMode="decimal",Jl.autocomplete="off";let _0=A("span","nq-cc-amount-suffix",TQ);_0.textContent="NIM";let E0=A("p","nq-cc-request-hint",Ol);U(E0,"shell.anyAmount");let SQ=A("p","nq-cc-request-link",Ol),rl=A("span","nq-cc-copy-wrap",Ol),Eh=A("button","nq-cc-request-copy",rl);Eh.type="button",U(Eh,"shell.copyLink");let fQ=A("span","nq-cc-copy-tooltip",rl);fQ.setAttribute("aria-hidden","true"),U(fQ,"shell.copied");function wQ(){let k=ol??Q?.account?.address;if(!k)return;let _=Number(Jl.value.replace(",",".")),H=Number.isFinite(_)&&_>0?kh(_):0n;SQ.textContent=fK(k,{amountLuna:H,basePath:h.requestLinkBase})}Jl.addEventListener("input",wQ);function F0(){if(!Q?.account)return;Jl.value="",wQ(),N0.textContent=K.t("shell.requestTitle",{ticker:"NIM"}),X.classList.add("nq-cc-show-request")}let yQ;Eh.addEventListener("click",()=>{let k=SQ.textContent;if(!k)return;try{navigator.clipboard.writeText(k)}catch{}rl.classList.add("nq-cc-copied"),clearTimeout(yQ),yQ=setTimeout(()=>rl.classList.remove("nq-cc-copied"),1200)});let ol=null,RQ;Ql.addEventListener("click",()=>{let k=ol??Q?.account?.address;if(!k)return;try{navigator.clipboard.writeText(k)}catch{}xl.classList.add("nq-cc-copied","nq-cc-copied-hold"),clearTimeout(RQ),RQ=setTimeout(()=>xl.classList.remove("nq-cc-copied"),800)}),Ql.addEventListener("blur",()=>xl.classList.remove("nq-cc-copied-hold")),Hl(D,"shell.sendTransaction",()=>Th());let Tl=A("div","nq-cc-sendto-body",D),Fh=A("div","nq-cc-field-head",Tl),Uh=A("span","nq-cc-recipient-icon",Fh),U0=A("label","nq-cc-eyebrow",Fh);U(U0,"shell.enterAddress");let qQ=A("div","nq-cc-addr-field",Tl);A("span","nq-cc-addr-rules",qQ);let b=A("textarea","nq-cc-addr-input",qQ);b.rows=3,b.placeholder=dl("NQ00000000000000000000000000000000000"),b.autocomplete="off",b.spellcheck=!1,b.setAttribute("aria-label",K.t("shell.recipient")),b.addEventListener("input",()=>aK(b)),b.addEventListener("keydown",(k)=>{if(k.key==="Enter")k.preventDefault()});let Pl=A("div","nq-cc-contacts-band",Tl);Pl.hidden=!0;let vQ=A("div","nq-cc-book",Pl);vQ.insertAdjacentHTML("beforeend",IZ);let H0=A("span","nq-cc-book-label",vQ);U(H0,"shell.contacts"),A("div","nq-cc-contacts-rule",Pl);let tl=A("div","nq-cc-contacts",Pl);tl.hidden=!0,Tl.insertBefore(Pl,Fh);let el=A("div","nq-cc-sendto-foot",Tl);if(el.hidden=!h.createCashlink&&!h.scan,h.createCashlink){let k=A("p","nq-cc-unavailable",el);U(k,"shell.addressUnavailable");let _=A("button","nq-cc-request-open",el);_.type="button",U(_,"shell.createCashlink"),_.addEventListener("click",()=>{B(!1),h.createCashlink()})}if(h.scan){let k=A("button","nq-cc-scan-open",el);k.type="button",k.setAttribute("aria-label","Scan QR code"),k.insertAdjacentHTML("beforeend",l0),k.addEventListener("click",()=>{B(!1),h.scan()})}async function L0(k){let _=h.contacts?.add;if(!_)return;let H=k.replace(/\s+/g,"").toUpperCase();try{if((await h.contacts.list()??[]).some((v)=>v.address.replace(/\s+/g,"").toUpperCase()===H))return;let y=window.prompt(K.t("shell.saveContact"))?.trim();if(!y)return;await _({label:y,address:k,asset:"NIM"})}catch{}}async function x0(){if(!h.contacts)return;let k=[];try{k=await h.contacts.list()??[]}catch{k=[]}let _=k.filter((S)=>(S.asset??"NIM")==="NIM");tl.textContent="";let H=_.slice(0,3);tl.hidden=H.length===0,Pl.hidden=H.length===0;for(let S of H){let y=A("button","nq-cc-contact",tl);y.type="button",y.title=S.address;let v=A("span","nq-cc-contact-icon",y);if(h.identicon)v.appendChild(h.identicon(S.address,40));else v.insertAdjacentHTML("beforeend",MQ);let g=A("span","nq-cc-contact-name",y);g.textContent=S.label,y.addEventListener("click",()=>{b.value=dl(S.address),fl(),b.focus(),b.setSelectionRange(0,0),pQ()})}}Hl(w,"shell.sendAmount",()=>{X.classList.remove("nq-cc-show-send"),X.classList.add("nq-cc-show-sendto")});let Kl=A("div","nq-cc-send-body",w),Hh=A("div","nq-cc-parties",Kl),BQ=A("div","nq-cc-party",Hh),bQ=A("span","nq-cc-party-icon",BQ),D0=A("span","nq-cc-party-name",BQ);A("span","nq-cc-party-dash",Hh);let gQ=A("div","nq-cc-party",Hh),mQ=A("span","nq-cc-party-icon",gQ),O0=A("span","nq-cc-party-name nq-cc-mono",gQ),T0=A("label","nq-cc-field-label",Kl);U(T0,"shell.amount");let dQ=A("div","nq-cc-amount-row",Kl),Ml=A("input","nq-cc-input",dQ);Ml.placeholder="0",Ml.inputMode="decimal",Ml.autocomplete="off";let S0=A("span","nq-cc-amount-suffix",dQ);S0.textContent="NIM";let Lh=A("p","nq-cc-send-fiat",Kl),o=A("input","nq-cc-message",Kl);o.type="text",o.autocomplete="off",o.placeholder=K.t("shell.publicMessage"),xQ.push(o),o.addEventListener("input",()=>{while(YZ(o.value)>64)o.value=[...o.value].slice(0,-1).join("")});let f0=A("p","nq-cc-send-hint",Kl),xh=A("p","nq-cc-send-error",Kl),jl=A("button","nq-cc-send-confirm",Kl);jl.type="button",U(jl,"shell.send");let uQ=A("div","nq-cc-send-done",w);uQ.insertAdjacentHTML("beforeend",'<svg width="36" height="36" viewBox="0 0 24 24" fill="none" aria-hidden="true"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2"/><path d="M7.5 12.5l3 3 6-6.5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" fill="none"/></svg>');let w0=A("span",void 0,uQ);U(w0,"shell.sent");let cQ=/^NQ\d{2}[0-9A-HJ-NP-VXY]{32}$/,iQ="";function y0(k){if(!h.identicon||iQ===(k??""))return;if(iQ=k??"",Uh.textContent="",Uh.style.visibility=k?"":"hidden",k)Uh.appendChild(h.identicon(k,36))}let Nl=()=>jh(b.value),Dh=()=>{let k=Number(Ml.value.replace(",","."));return Number.isFinite(k)?k:0},Sl=null;function Oh(k){if(!Y||Sl===null){Lh.hidden=!0;return}Lh.hidden=!1,Lh.textContent=Al(k*Sl,P)}async function R0(){if(!Y){Oh(0);return}try{Sl=await h.fiat.rate(P)}catch{Sl=null}Oh(Dh())}function fl(){let k=cQ.test(Nl());y0(k?Nl():null);let _=Dh();Oh(_);let H=_>0&&(i===null||_<=El(i));jl.disabled=!(k&&H)}b.addEventListener("input",fl),Ml.addEventListener("input",fl);function q0(){let k=Q?.account;if(!k)return;D0.textContent=k.label||dl(k.address).slice(0,9);let _=Nl().match(/.{1,4}/g)??[];if(O0.textContent=_.slice(0,3).join(" "),!h.identicon)return;bQ.textContent="",bQ.appendChild(h.identicon(k.address,56)),mQ.textContent="",mQ.appendChild(h.identicon(Nl(),56))}function pQ(){if(!cQ.test(Nl()))return;if(X.classList.contains("nq-cc-show-send"))return;q0(),X.classList.remove("nq-cc-show-sendto"),X.classList.add("nq-cc-show-send"),Ml.focus()}b.addEventListener("input",pQ);function v0(){if(!Q?.account)return;xh.textContent="",w.classList.remove("nq-cc-sent"),f0.textContent=i!==null?`${K.t("shell.available")}: ${Yh(i)} NIM`:"",fl(),R0(),X.classList.add("nq-cc-show-sendto"),x0()}function Th(){X.classList.remove("nq-cc-show-send"),X.classList.remove("nq-cc-show-sendto")}jl.addEventListener("click",async()=>{if(!Q)return;let _=Nl().replace(/(.{4})(?=.)/g,"$1 ");jl.disabled=!0,xh.textContent="",jl.textContent=K.t("shell.sending");try{let H=o.value.trim();if(await Q.pay({recipient:_,valueLuna:kh(Dh()),...H?{data:H}:{}}))w.classList.add("nq-cc-sent"),L0(_),b.value="",Ml.value="",lh=0,window.setTimeout(()=>{Th(),yl(!0)},1800);else Th()}catch(H){if(!/cancel|denied|rejected|closed|dismiss/i.test(String(H)))xh.textContent=K.t("shell.sendFailed")}finally{jl.textContent=K.t("shell.send"),o.value="",fl()}});let wl="",Zl=null;m=()=>{let k=Zl?Zl.ticker:"NIM";M0.textContent=`${K.t("shell.receive")} ${k}`,$0.textContent=K.t("shell.addressSheet",{ticker:k}),J0.textContent=K.t("shell.scanToSend",{ticker:k}),Ch.hidden=!Zl,Ch.textContent=Zl?K.t("shell.networkOnly",{ticker:Zl.ticker,network:Zl.network}):""};function nQ(k){let _=Q?.account??null;if(!_)return;if(k&&!k.address)return;let H=k?.address??_.address;if(!H)return;X.classList.add("nq-cc-show-receive");let S=H.replace(/\s+/g,"");ol=S,Ql.textContent="";let y=Z0(S);Ql.style.setProperty("--nq-cc-addr-cols",String(y.columns));for(let p of y.cells){let n=A("span",void 0,Ql);n.textContent=p}Zl=k??null,m();let v=!!h.identicon;if(nl.hidden=!v,sl.hidden=!v,al.hidden=!!k,Nh.hidden=v,v)nl.textContent="",nl.appendChild(h.identicon(H,100));I0.textContent=P0(S);let g=k?k.uri?.(S)??S:`nimiq:${S}`;if(wl!==g){let p=v?A0:Nh,n=v?200:164;try{p.textContent="",p.appendChild(h.qr?h.qr(g,n):sK(g,n,p)),wl=g}catch{p.textContent="",wl=""}}}function sQ(k,_){k.addEventListener("click",()=>{let H=_.classList.toggle("nq-cc-open");k.setAttribute("aria-expanded",String(H))})}function Sh(k,_){_.classList.remove("nq-cc-open"),k.setAttribute("aria-expanded","false")}sQ(Vl,Ih);let lh=0,i=null;async function yl(k=!1){let _=Q?.account??null;if(!G||!_)return;if(e){await e.refresh(k);let S=e.total();kl.hidden=S===null,jQ.textContent=S===null?"":Al(S,P),Il.hidden=!0;let y=e.units("NIM");if(y!==null)i=Number(y);if(!M)return}let H=Date.now();if(!k&&H-lh<30000&&i!==null){aQ();return}try{i=await M(_.address),lh=H}catch{}if(aQ(),Y&&!e&&i!==null)try{let S=await h.fiat.rate(P);if(S!==null&&Q?.account)Il.textContent=Al(El(i)*S,P),Il.hidden=!1;else Il.hidden=!0}catch{Il.hidden=!0}}function aQ(){if(e)return;if(i===null){kl.hidden=!0;return}if(kl.hidden=!1,jQ.textContent=`${Yh(i)} NIM`,!Y)Il.hidden=!0}function rQ(){let k=Q?.account??null;if(!k)return;if(Ah.textContent="",h.identicon)Ah.appendChild(h.identicon(k.address,40));else Ah.innerHTML=MQ;cl.textContent=h0(k,K.t("shell.account")),wl="",yl()}function fh(){J.style.setProperty("--nq-cc-menu-shift","0px");let k=J.getBoundingClientRect(),_=K0(k.left,k.right,window.innerWidth);if(_)J.style.setProperty("--nq-cc-menu-shift",`${_}px`)}function B(k){if(J.hidden=!k,I.setAttribute("aria-expanded",String(k)),$.setAttribute("aria-expanded",String(k)),k)fh(),window.addEventListener("resize",fh),document.addEventListener("click",oQ,!0),document.addEventListener("keydown",tQ),yl();else X.classList.remove("nq-cc-show-receive"),X.classList.remove("nq-cc-show-qr"),X.classList.remove("nq-cc-show-request"),X.classList.remove("nq-cc-show-sendto"),X.classList.remove("nq-cc-show-send"),window.removeEventListener("resize",fh),document.removeEventListener("click",oQ,!0),document.removeEventListener("keydown",tQ)}function oQ(k){if(!X.contains(k.target))B(!1)}function tQ(k){if(k.key==="Escape")B(!1)}let eQ=()=>B(J.hidden);if(I.addEventListener("click",eQ),$.addEventListener("click",eQ),C(),L(),NQ(),_Q(),Y)h.fiat.onChange?.(P);if(Q?.account)rQ();let lK=Q?.account?.address??null,B0=Q?Q.onAccountChange(()=>{let k=Q.account?.address??null;if(k!==lK)lK=k,i=null,lh=0,e?.clear(),X.classList.remove("nq-cc-show-receive"),Zl=null,ol=null,wl="",Ql.textContent="",Nh.textContent="";if(C(),Q.account)rQ(),yl(!0);else kl.hidden=!0}):()=>{},b0=K.onChange(()=>{a();for(let k of HQ)k.setAttribute("aria-label",K.t("shell.back"));for(let k of LQ)k.setAttribute("aria-label",K.t("shell.close"));for(let k of DQ)k.setAttribute("aria-label",K.t("shell.showQr"));for(let k of xQ)k.placeholder=K.t("shell.publicMessage");NQ(),L(),C()});return{el:X,get fiatTicker(){return Y?P:null},open:()=>B(!0),close:()=>B(!1),destroy(){B0(),b0(),B(!1),e?.destroy(),X.remove()}}}var FZ=X0;export{mK as validateFeedbackInput,vK as themeVars,gK as submitToBot,dK as submitFeedback,x1 as shellLocales,ml as scrubAddresses,_1 as parseNim,ZQ as pageContext,kQ as openReportBugSheet,kh as nimToLuna,p1 as mountWalletPill,lQ as mountProfileWidget,X0 as mountMiniWallet,B1 as mountLanguageSwitcher,m1 as mountLanguagePill,FZ as mountCornerControl,VQ as mountAssetList,D1 as mergeLocales,K0 as menuShift,El as lunaToNim,wh as isMiniAppHost,KQ as installReportCapture,yh as hasNimiqProvider,Gh as fmtUnits,Yh as fmtNim,Al as fmtFiat,oh as flagDataUrl,Vh as donutPoint,w1 as donutArcs,hh as detectModeSync,GQ as defaultReportBugRepo,M1 as createWallet,ah as createNimBalanceReader,L1 as createI18n,uK as collectDiagnostics,s as buildFlagHex,f1 as areaPaths,Ul as applyTheme,Z0 as addressGrid,rh as SHELL_LANGUAGES,I1 as NIM_DECIMALS,ql as MiniAppBackend,A1 as LUNA_PER_NIM,Bl as HubBackend,LK as FLAG_SVG,th as FLAG_FIT,bl as FEATURED_LANGUAGES,j1 as DEFAULT_NIM_RPC};
