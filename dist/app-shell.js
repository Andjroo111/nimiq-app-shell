...
</svg>`};function EZ(Q){let Z=E0[Q.toLowerCase()];return Z?"data:image/svg+xml,"+encodeURIComponent(Z):""}var AZ={us:{aspect:1.3333333333333333},cn:{scale:1.2,dx:2.2,dy:2.8},kr:{scale:0.72}};var A0="M19.964 8.156 15.758.844A1.69 1.69 0 0014.299 0H5.887c-.6 0-1.156.32-1.456.844L.225 8.156c-.3.523-.3 1.165 0 1.688l4.206 7.312c.3.523.856.844 1.456.844h8.412c.6 0 1.156-.32 1.456-.844l4.206-7.312a1.69 1.69 0 00.003-1.688",s1=0;function a1(Q){let Z=Q?.aspect??1,z=Q?.scale??1,X=Q?.dx??0,K=Q?.dy??0,Y=1.08,V=Math.max(20,18*Z)*1.08*z,W=V/Z;return{x:10+X-V/2,y:9+K-W/2,w:V,h:W}}function t1(Q,Z={}){let z=Z.size??24,X=Z.fit??AZ[Q.toLowerCase()],{x:K,y:Y,w:V,h:W}=a1(X),$=Math.max(0.4,22/z),_=`nq-flag-${s1+=1}`;return`<svg class="nq-flag-hex" viewBox="0 0 20 18" width="${z}" height="${(z*0.9).toFixed(2)}" aria-hidden="true" style="display:block;overflow:visible"><defs><clipPath id="${_}"><path d="${A0}"/></clipPath></defs><g clip-path="url(#${_})"><image href="${EZ(Q)}" x="${K}" y="${Y}" width="${V}" height="${W}" preserveAspectRatio="xMidYMid slice"/></g><path d="${A0}" fill="none" stroke="rgba(31,35,72,0.4)" stroke-width="${$.toFixed(2)}" stroke-linejoin="round"/></svg>`}function B(Q,Z={}){let z=document.createElement("div");return z.innerHTML=t1(Q,Z),z.firstElementChild}function e1(Q,Z,z,X={}){let K=X.pad??2,Y=X.bandDepth??14;if(Q.length===0)return{line:"",fill:"",band:""};let V=Math.max(...Q),W=Math.min(...Q),$=V-W,_=z-K*2,N=(x)=>Q.length===1?Z/2:x/(Q.length-1)*Z,A=(x)=>$===0?K+_/2:K+_-(x-W)/$*_,J=Q.map((x,U)=>[N(U),A(x)]),H=`M${J.map(([x,U])=>`${x.toFixed(1)},${U.toFixed(1)}`).join("L")}`,M=`${H}L${Z.toFixed(1)},${z}L0,${z}Z`,C=J.slice().reverse().map(([x,U])=>`${x.toFixed(1)},${(U+Y).toFixed(1)}`),O=`${H}L${C.join("L")}Z`;return{line:H,fill:M,band:O}}function BQ(Q,Z,z,X){let K=Q*Math.PI*2-Math.PI/2;return[z+Z*Math.cos(K),X+Z*Math.sin(K)]}function Q4(Q,Z,z,X){let K=Q.reduce((W,$)=>W+($>0?$:0),0);if(K<=0)return[];let Y=[],V=0;return Q.forEach((W,$)=>{if(W<=0)return;let _=W/K,[N,A]=BQ(V,Z,z,X),[J,P]=BQ(V+_,Z,z,X),H=_>=1?BQ(0.9999,Z,z,X):[J,P],M=_>0.5?1:0;Y.push({path:`M${N.toFixed(2)},${A.toFixed(2)}A${Z},${Z} 0 ${M} 1 ${H[0].toFixed(2)},${H[1].toFixed(2)}`,fraction:_,index:$}),V+=_}),Y}var xZ="data:image/svg+xml,"+encodeURIComponent('<svg width="64" height="64" viewBox="0 -4 64 64" xmlns="http://www.w3.org/2000/svg"><path opacity=".1" d="M62.3 25.4L49.2 2.6A5.3 5.3 0 0 0 44.6 0H18.4c-1.9 0-3.6 1-4.6 2.6L.7 25.4c-1 1.6-1 3.6 0 5.2l13.1 22.8c1 1.6 2.7 2.6 4.6 2.6h26.2c1.9 0 3.6-1 4.6-2.6l13-22.8c1-1.6 1-3.6.1-5.2z" fill="#1F2348"/></svg>');function Z4(Q){let Z=(Q??"").trim(),z=Z.split(/\s+/);if(z.length>=5)return`${z.slice(0,3).join(" ")} … ${z.slice(-1)}`;if(Z.length>16)return`${Z.slice(0,8)}…${Z.slice(-5)}`;return Z}function z4(){if(typeof document>"u")return;if(document.getElementById("nimiq-shell-profile-style"))return;let Q=document.createElement("style");Q.id="nimiq-shell-profile-style",Q.textContent=`
...
`,document.head.appendChild(Q)}function Y4(Q,Z){let{i18n:z}=Z,X=Z.languages??CZ,K=Z.size??40;if(Z.injectStyles!==!1)K4();let Y=document.createElement("ul");Y.className="nq-langsw",Y.setAttribute("role","listbox"),Y.setAttribute("aria-label",z.t("shell.language")),Y.style.setProperty("--nq-flag-w",`${K}px`);let V=new Map;for(let _ of X){let N=document.createElement("li"),A=document.createElement("button");A.type="button",A.className="nq-langsw__btn",A.setAttribute("role","option"),A.setAttribute("aria-label",_.name);let J=document.createElement("span");J.className="nq-langsw__art",J.appendChild(B(_.flag,{size:K}));let P=document.createElement("span");P.className="nq-langsw__tip",P.setAttribute("aria-hidden","true"),P.textContent=_.name,A.appendChild(J),A.appendChild(P),A.addEventListener("click",()=>z.setLanguage(_.id)),N.appendChild(A),Y.appendChild(N),V.set(_.id,A)}function W(_){for(let[N,A]of V){let J=N===_;A.classList.toggle("is-active",J),A.setAttribute("aria-selected",String(J))}}W(z.getLanguage());let $=z.onChange((_)=>W(_));return Q.appendChild(Y),{el:Y,destroy(){$(),Y.remove()}}}var U0="nimiq-shell-langpill-style";function V4(){if(typeof document>"u"||document.getElementById(U0))return;let Q=document.createElement("style");Q.id=U0,Q.textContent=`
...
`,document.head.appendChild(Q)}var W4='<svg class="nq-langpill__caret" viewBox="0 0 10 6" aria-hidden="true"><path d="M1 1l4 4 4-4" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>';function G4(Q,Z){let{i18n:z}=Z,X=Z.languages??FQ,K=Z.size??24;if(Z.injectStyles!==!1)V4();let Y=document.createElement("div");Y.className="nq-langpill";let V=document.createElement("button");V.type="button",V.className="nq-langpill__btn",V.setAttribute("aria-haspopup","listbox"),V.setAttribute("aria-expanded","false"),V.setAttribute("aria-label",z.t("shell.language"));let W=document.createElement("ul");W.className="nq-langpill__menu",W.setAttribute("role","listbox"),W.setAttribute("aria-label",z.t("shell.language")),W.hidden=!0;let $=new Map;for(let C of X){let O=document.createElement("li"),x=document.createElement("button");x.type="button",x.className="nq-langpill__option",x.setAttribute("role","option"),x.appendChild(B(C.flag,{size:K}));let U=document.createElement("span");U.className="nq-langpill__name",U.textContent=C.name,x.appendChild(U),x.addEventListener("click",()=>{z.setLanguage(C.id),J()}),O.appendChild(x),W.appendChild(O),$.set(C.id,x)}Y.appendChild(V),Y.appendChild(W),Q.appendChild(Y);function _(){let C=z.getLanguage(),O=X.find((x)=>x.id===C)??X[0];if(V.textContent="",O)V.appendChild(B(O.flag,{size:K}));V.insertAdjacentHTML("beforeend",W4)}function N(C){for(let[O,x]of $){let U=O===C;x.classList.toggle("is-active",U),x.setAttribute("aria-selected",String(U))}}function A(){W.hidden=!1,V.setAttribute("aria-expanded","true"),document.addEventListener("click",P,!0),document.addEventListener("keydown",H)}function J(){if(W.hidden)return;W.hidden=!0,V.setAttribute("aria-expanded","false"),document.removeEventListener("click",P,!0),document.removeEventListener("keydown",H)}function P(C){if(!Y.contains(C.target))J()}function H(C){if(C.key==="Escape")J(),V.focus()}V.addEventListener("click",()=>W.hidden?A():J()),_(),N(z.getLanguage());let M=z.onChange((C)=>{_(),N(C)});return{el:Y,destroy(){M(),J(),Y.remove()}}}var H0="nimiq-shell-walletpill-style";function $4(){if(typeof document>"u"||document.getElementById(H0))return;let Q=document.createElement("style");Q.id=H0,Q.textContent=`
...
`,document.head.appendChild(Q)}var j4="data:image/svg+xml,"+encodeURIComponent('<svg width="64" height="64" viewBox="0 -4 64 64" xmlns="http://www.w3.org/2000/svg"><path opacity=".25" d="M62.3 25.4L49.2 2.6A5.3 5.3 0 0 0 44.6 0H18.4c-1.9 0-3.6 1-4.6 2.6L.7 25.4c-1 1.6-1 3.6 0 5.2l13.1 22.8c1 1.6 2.7 2.6 4.6 2.6h26.2c1.9 0 3.6-1 4.6-2.6l13-22.8c1-1.6 1-3.6.1-5.2z" fill="currentColor"/></svg>'),HZ='<svg class="nq-connect__icon" viewBox="0 0 20 20" fill="none" aria-hidden="true"><rect x="2" y="5" width="16" height="12" rx="2" stroke="currentColor" stroke-width="1.5"/><path d="M2 9h16" stroke="currentColor" stroke-width="1.5"/><circle cx="6" cy="13" r="1" fill="currentColor"/></svg>',_4='<svg class="nq-wallet__caret" viewBox="0 0 10 6" aria-hidden="true"><path d="M1 1l4 4 4-4" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>';function J4(Q,Z){if(Q.label)return Q.label;let z=Q.address?.trim()??"";if(!z)return Z;return`${z.slice(0,7)}…${z.slice(-4)}`}function P4(Q,Z){let{wallet:z,i18n:X}=Z;if(Z.injectStyles!==!1)$4();let K=document.createElement("div");K.className="nq-wallet",Q.appendChild(K);let Y=null,V=!1;function W(){Y?.destroy(),Y=null,document.removeEventListener("click",$,!0),document.removeEventListener("keydown",_)}function $(C){if(!K.contains(C.target))N()}function _(C){if(C.key==="Escape")N()}function N(){if(!V)return;V=!1,P()}function A(){let C=document.createElement("button");C.className="nq-connect",C.type="button",C.innerHTML=HZ+`<span>${X.t("shell.connectWallet")}</span>`,C.addEventListener("click",async()=>{C.disabled=!0,C.innerHTML=HZ+`<span>${X.t("shell.connecting")}</span>`;try{await z.connect()}catch{C.disabled=!1,C.innerHTML=HZ+`<span>${X.t("shell.retry")}</span>`}}),K.appendChild(C)}function J(){let C=z.account,O=document.createElement("button");O.className="nq-wallet__btn",O.type="button",O.setAttribute("aria-haspopup","dialog"),O.setAttribute("aria-expanded",String(V));let x=document.createElement("span");if(x.className="nq-wallet__icon",Z.identicon)x.appendChild(Z.identicon(C.address,28));else{let f=document.createElement("img");f.src=j4,f.alt="",x.appendChild(f)}O.appendChild(x);let U=document.createElement("span");if(U.className="nq-wallet__label",U.textContent=J4(C,X.t("shell.account")),O.appendChild(U),O.insertAdjacentHTML("beforeend",_4),O.addEventListener("click",()=>{V=!V,P()}),K.appendChild(O),V){let f=document.createElement("div");f.className="nq-wallet__menu",K.appendChild(f),Y=UZ(f,{wallet:z,i18n:X,identiconSize:40,identicon:Z.identicon,showCopy:!0,showDisconnect:!0}),document.addEventListener("click",$,!0),document.addEventListener("keydown",_)}}function P(){if(W(),K.textContent="",z.account)J();else A()}P();let H=z.onAccountChange(()=>{V=!1,P()}),M=X.onChange(()=>{if(!z.account)P()});return{el:K,destroy(){H(),M(),W(),K.remove()}}}var yQ=[],OZ=[],O0=!1;function NQ(Q,Z,z){if(Q.push(Z),Q.length>z)Q.shift()}function I4(Q){try{return typeof Q==="string"?Q:JSON.stringify(Q)}catch{return String(Q)}}function L0(Q){return Q?`
`+String(Q).split(`
`).slice(0,4).join(`
`):""}function D0(Q){try{let Z=new URL(Q,location.href);return Z.pathname+Z.search}catch{return String(Q).slice(0,120)}}function k0(Q){let Z=Q.search(/[?#]/);return Z===-1?Q:Q.slice(0,Z)}function LZ(Q){if(O0||typeof window>"u")return;O0=!0,window.addEventListener("error",(z)=>{let X=z.filename?`
  at ${z.filename}:${z.lineno}:${z.colno}`:"";NQ(yQ,(z.message||"Error")+X+L0(z.error?.stack),20)}),window.addEventListener("unhandledrejection",(z)=>{let X=z.reason??{};NQ(yQ,`Unhandled rejection: ${X.message??String(z.reason)}${L0(X.stack)}`,20)});let Z=console.error.bind(console);if(console.error=(...z)=>{try{NQ(yQ,z.map(I4).join(" "),20)}catch{}Z(...z)},typeof window.fetch==="function"){let z=window.fetch,X=z.bind(window),K=async(Y,V)=>{let W=typeof Y==="string"?Y:Y instanceof URL?Y.href:Y.url,$=V?.method??(Y instanceof Request?Y.method:"GET"),_=Boolean(Q)&&W.startsWith(Q);try{let N=await X(Y,V);if(N.status>=400&&!_)NQ(OZ,`${$} ${D0(W)} → ${N.status}`,12);return N}catch(N){if(!_)NQ(OZ,`${$} ${D0(W)} → network error`,12);throw N}};window.fetch=Object.assign(K,z)}}function DZ(Q=!1){return{url:typeof location<"u"?k0(location.href):"",title:typeof document<"u"?document.title:"",referrer:typeof document<"u"?k0(document.referrer):"",userAgent:typeof navigator<"u"?navigator.userAgent:"",viewport:typeof window<"u"?{w:window.innerWidth,h:window.innerHeight,dpr:window.devicePixelRatio||1}:{w:0,h:0,dpr:1},consoleErrors:yQ.slice(-12),networkFailures:OZ.slice(-10),hasScreenshot:Q}}var M4="https://bot.nimiq.tech";function CQ(Q){return Q.replace(/NQ\d{2}[\s]?(?:[0-9A-HJ-NP-VXY]{4}[\s]?){8}/gi,"[address redacted]").replace(/(^|[^0-9a-f])([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})(?![0-9a-f])/gi,"$1[id redacted]")}function kZ(Q){if(typeof Q==="string")return CQ(Q);if(Array.isArray(Q))return Q.map(kZ);if(Q&&typeof Q==="object"){let Z={};for(let[z,X]of Object.entries(Q))Z[z]=kZ(X);return Z}return Q}async function h0(Q,Z){let z=(Q.service??M4).replace(/\/$/,""),X=kZ({...Z.pageContext??{},...Z.context??{}}),K=CQ([`[${Z.type}] ${Z.title.trim()}`,"",Z.description.trim(),...F4(Z)].join(`
`)),Y=async(V,W)=>{let $=await globalThis.fetch(`${z}${V}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(W)}),_={};try{_=await $.json()}catch{}return{res:$,json:_}};try{let V=await Y("/api/draft",{repo:Q.repo,text:K,context:X});if(!V.res.ok)return{ok:!1,status:V.res.status,error:f0(V.json)};let W=V.json.draft,$=V.json.reportId;if(!W||!$)return{ok:!1,status:V.res.status,error:"The issue service sent back nothing to file."};let _=[...new Set([...W.labels??[],...Q.labels??[]])],N={reportId:$,repo:Q.repo,title:CQ(W.title),body:CQ(W.body)},A=await Y("/api/file",{...N,labels:_});if(!A.res.ok&&_.length&&String(A.json.error??"")==="github_failed")A=await Y("/api/file",{...N,labels:[]});let J=A.json.url;if(!A.res.ok&&!J)return{ok:!1,status:A.res.status,error:f0(A.json)};return{ok:!0,status:A.res.status,issueNumber:A.json.number,issueUrl:J}}catch(V){return{ok:!1,status:0,error:V instanceof Error?V.message:String(V)}}}function F4(Q){let Z=Q.context??{},z=Object.entries(Z).map(([X,K])=>`${X}: ${K}`).join(" · ");return z?["","---",z]:[]}function f0(Q){let Z=String(Q.error??"");return{rate_limited:"Too many reports just now. Give it a minute.",unknown_repo:"The issue service doesn't know this app.",empty_report:"Please describe the problem first.",github_not_configured:"The issue service isn't connected to GitHub yet.",github_failed:"GitHub rejected the issue. Try again shortly.",already_filed:"That report was already filed."}[Z]??"Something went wrong."}function S0(Q){let Z=["bug","idea","question"];if(!Q.type||!Z.includes(Q.type))return"shell.fbErrType";if((Q.title??"").trim().length<5)return"shell.fbErrTitle";if((Q.description??"").trim().length<10)return"shell.fbErrDetails";return null}async function q0(Q,Z){let z={...Z.context??{},type:Z.type,title:Z.title.trim(),description:Z.description.trim()};if(Z.diagnostic)z.diagnostic=Z.diagnostic;let X;try{X=await globalThis.fetch(Q,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(z)})}catch(Y){return{ok:!1,status:0,error:Y instanceof Error?Y.message:String(Y)}}let K={};try{K=await X.json()}catch{}if(X.ok)return{ok:!0,status:X.status,issueNumber:K.issueNumber};return{ok:!1,status:X.status,error:K.error??`Server returned ${X.status}.`,fallbackMailto:K.fallbackMailto}}function w0(){let Q=[];try{if(typeof location<"u")Q.push(`page: ${location.pathname}${location.hash}`);if(typeof navigator<"u")Q.push(`ua: ${navigator.userAgent}`),Q.push(`lang: ${navigator.language}`);if(typeof window<"u")Q.push(`viewport: ${window.innerWidth}×${window.innerHeight}`)}catch{}return Q.join(`
`)}var T0="nimiq-shell-report-bug-style",fZ='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 20a6 6 0 0 0 6-6v-3a6 6 0 0 0-12 0v3a6 6 0 0 0 6 6Z"/><path d="M10 6.5 8.5 4M14 6.5 15.5 4"/><path d="M12 8v12"/><path d="M6 11H3M6 15H3M6.5 18.5 4 20M18 11h3M18 15h3M17.5 18.5 20 20"/></svg>';function N4(Q){if(Q.getElementById(T0))return;let Z=Q.createElement("style");Z.id=T0,Z.textContent=`
...
`,Q.head.appendChild(Z)}function b(Q){return Q.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function C4(Q,Z){let z=Q.createElement("div");z.className="nq-fb-toast",z.setAttribute("role","status"),z.textContent=Z,Q.body.appendChild(z),setTimeout(()=>z.remove(),3000)}function TZ(Q,Z,z){if(Q.getElementById("nq-fb-scrim"))return;N4(Q);let X=(M)=>Z.t(M),K=z.diagnostics!==!1,Y=Q.createElement("div");Y.id="nq-fb-scrim",Y.className="nq-fb-scrim",Y.innerHTML=`
    <div class="nq-fb-card" role="dialog" aria-modal="true" aria-labelledby="nq-fb-title">
      <div class="nq-fb-head">${fZ}<h2 class="nq-fb-title" id="nq-fb-title">${b(X("shell.reportBug"))}</h2></div>
      <label class="nq-fb-field">
        <span class="nq-fb-label">${b(X("shell.fbType"))}</span>
        <select class="nq-fb-input" id="nq-fb-type">
          <option value="bug">${b(X("shell.fbBug"))}</option>
          <option value="idea">${b(X("shell.fbIdea"))}</option>
          <option value="question">${b(X("shell.fbQuestion"))}</option>
        </select>
      </label>
      <label class="nq-fb-field">
        <span class="nq-fb-label">${b(X("shell.fbSummary"))}</span>
        <input class="nq-fb-input" type="text" id="nq-fb-summary" maxlength="120" autocomplete="off" />
      </label>
      <label class="nq-fb-field">
        <span class="nq-fb-label">${b(X("shell.fbDetails"))}</span>
        <textarea class="nq-fb-input" id="nq-fb-details" maxlength="4000"></textarea>
      </label>
      ${K?`<label class="nq-fb-diag">
        <input type="checkbox" id="nq-fb-diag" checked />${b(X("shell.fbIncludeDiag"))}
      </label>`:""}
      <p class="nq-fb-error" id="nq-fb-error" role="alert"></p>
      <div class="nq-fb-actions">
        <button type="button" class="nq-fb-cancel" id="nq-fb-cancel">${b(X("shell.cancel"))}</button>
        <button type="button" class="nq-fb-send" id="nq-fb-send">${b(X("shell.fbSend"))}</button>
      </div>
    </div>`,Q.body.appendChild(Y);let V=(M)=>Y.querySelector(`#${M}`),W=V("nq-fb-type"),$=V("nq-fb-summary"),_=V("nq-fb-details"),N=K?V("nq-fb-diag"):null,A=V("nq-fb-error"),J=V("nq-fb-send"),P=(M)=>{if(M.key==="Escape")H()};function H(){Y.remove(),Q.removeEventListener("keydown",P)}V("nq-fb-cancel").addEventListener("click",H),Y.addEventListener("pointerdown",(M)=>{if(M.target===Y)H()}),Q.addEventListener("keydown",P),$.focus();for(let M of[W,$,_])M.addEventListener("input",()=>{A.style.display="none"});J.addEventListener("click",async()=>{let M={type:W.value||"",title:$.value,description:_.value,context:z.context};if(N?.checked)if(z.bot)M.pageContext=DZ();else M.diagnostic=w0();let C=S0(M);if(C){A.textContent=X(C),A.style.display="block";return}J.disabled=!0,J.textContent=X("shell.fbSending"),A.style.display="none";let O=z.bot?await h0(z.bot,M):await q0(z.endpoint,M);if(J.disabled=!1,J.textContent=X("shell.fbSend"),O.ok){H(),C4(Q,X("shell.fbThanks")),z.onSubmitted?.(O);return}let x=O.error??X("shell.fbFailed");A.innerHTML=O.fallbackMailto?`${b(x)} <a href="${b(O.fallbackMailto)}">${b(X("shell.fbFailEmail"))}</a>`:b(x),A.style.display="block"})}function E4(Q){let Z=Q.name?.trim(),z=Q.network.trim();if(!Z)return z;if(Z.toLowerCase()===z.toLowerCase())return Z;return`${Z} · ${z}`}var b0="nimiq-shell-asset-list-style";function A4(){if(typeof document>"u"||document.getElementById(b0))return;let Q=document.createElement("style");Q.id=b0,Q.textContent=`
...
`,document.head.appendChild(Q)}function l(Q,Z,z){let X=document.createElement(Q);if(Z)X.className=Z;if(z)z.appendChild(X);return X}function hZ(Q,Z){if(Z.injectStyles!==!1)A4();let z=Z.cacheMs??30000,X=()=>typeof Z.assets==="function"?Z.assets():Z.assets,K=l("div","nq-al");Q.appendChild(K);let Y=new Map,V="",W=!1;function $(J){let P=J.map((M)=>`${M.ticker}${M.address?"+":"-"}`).join(" ");if(P===V)return;V=P,K.textContent="";let H=new Map;for(let M of J){let C=typeof Z.onSelect==="function"&&!!M.address,O=l(C?"button":"div","nq-al-row",K);if(C)O.type="button",O.addEventListener("click",()=>Z.onSelect(M));let x=l("span","nq-al-art",O);if(M.icon)x.appendChild(M.icon(26));let U=l("span","nq-al-id",O),f=l("span","nq-al-tick",U);f.textContent=M.ticker;let EQ=l("span","nq-al-name",U);EQ.textContent=E4(M);let T=l("span","nq-al-amt",O),ZQ=l("span","nq-al-units nq-al-pending",T);ZQ.textContent="—";let mQ=l("span","nq-al-fiat",T),zQ=Y.get(M.ticker),R={units:zQ?.units??null,fiat:zQ?.fiat??null,fetchedAt:zQ?.fetchedAt??0,unitsEl:ZQ,fiatEl:mQ};if(H.set(M.ticker,R),R.units!==null)_(M,R)}Y.clear();for(let[M,C]of H)Y.set(M,C)}function _(J,P){if(P.units===null)return;if(P.unitsEl.classList.remove("nq-al-pending"),P.unitsEl.textContent=bQ(P.units,J.decimals,{maxDecimals:J.maxDecimals??J.decimals}),P.fiat===null){P.fiatEl.textContent="";return}let H=Z.fiatTicker?.()??"USD";try{P.fiatEl.textContent=WQ(P.fiat,H)}catch{P.fiatEl.textContent=""}}async function N(J,P,H){let M=Y.get(J.ticker);if(!M)return;if(!P&&M.units!==null&&H-M.fetchedAt<z){_(J,M);return}try{let C=await J.balance();if(W)return;if(C!==null&&C!==void 0)M.units=typeof C==="bigint"?C:BigInt(Math.round(C)),M.fetchedAt=H}catch{}if(W)return;if(_(J,M),Z.rate&&M.units!==null){try{let C=await Z.rate(J.ticker);if(W)return;M.fiat=C===null?null:Number(M.units)/10**J.decimals*C}catch{M.fiat=null}if(!W)_(J,M)}}async function A(J=!1){if(W)return;let P=X();$(P);let H=Date.now();await Promise.all(P.map((M)=>N(M,J,H)))}if($(X()),Z.autoRefresh!==!1)A();return{el:K,refresh:A,total(){let J=null;for(let P of Y.values()){if(P.fiat===null)continue;J=(J??0)+P.fiat}return J},units:(J)=>Y.get(J)?.units??null,clear(){for(let J of Y.values())J.units=null,J.fiat=null,J.fetchedAt=0,J.unitsEl.classList.add("nq-al-pending"),J.unitsEl.textContent="—",J.fiatEl.textContent=""},destroy(){W=!0,K.remove()}}}var x4=/^NQ[0-9]{2}[0-9A-HJ-NP-VXY]{32}$/;function d0(Q){let Z=Q.replace(/\s+/g,"");if(!Z)return{cells:[],columns:1};if(x4.test(Z.toUpperCase()))return{cells:Z.match(/.{4}/g)??[Z],columns:3};let z=Z.length,X=Math.floor(z/3),K=z%3,Y=[],V=0;for(let W=0;W<3;W+=1){let $=X+(W<K?1:0);if($===0)continue;Y.push(Z.slice(V,V+$)),V+=$}return{cells:Y,columns:1}}var R0="nimiq-shell-corner-control-style",v0="nq-shell:fiat",l0="nq-shell:label:";function U4(Q){try{return localStorage.getItem(l0+Q.replace(/\s+/g,""))}catch{return null}}function H4(Q,Z){try{localStorage.setItem(l0+Q.replace(/\s+/g,""),Z)}catch{}}var O4={en:"English",es:"Español",de:"Deutsch",fr:"Français",pt:"Português",hi:"हिन्दी",zh:"中文",tr:"Türkçe",ko:"한국어",vi:"Tiếng Việt",ha:"Hausa"},B0={AED:"ae",ARS:"ar",AUD:"au",BRL:"br",CAD:"ca",CHF:"ch",CLP:"cl",CNY:"cn",CRC:"cr",CZK:"cz",DKK:"dk",EUR:"eu",GBP:"gb",GMD:"gm",GTQ:"gt",HKD:"hk",HUF:"hu",IDR:"id",ILS:"il",INR:"in",JPY:"jp",KRW:"kr",MXN:"mx",MYR:"my",NGN:"ng",NOK:"no",NZD:"nz",PHP:"ph",PKR:"pk",PLN:"pl",RUB:"ru",SEK:"se",SGD:"sg",THB:"th",TRY:"tr",TWD:"tw",UAH:"ua",USD:"us",VND:"vn",ZAR:"za"};function L4(){if(typeof document>"u"||document.getElementById(R0))return;let Q=document.createElement("style");Q.id=R0,Q.textContent=`
...
`,document.head.appendChild(Q)}var gQ='<svg class="nq-cc-caret" viewBox="0 0 10 6" aria-hidden="true"><path d="M1 1l4 4 4-4" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>',y0='<svg viewBox="0 -4 64 64" aria-hidden="true"><path opacity=".25" d="M62.3 25.4L49.2 2.6A5.3 5.3 0 0 0 44.6 0H18.4c-1.9 0-3.6 1-4.6 2.6L.7 25.4c-1 1.6-1 3.6 0 5.2l13.1 22.8c1 1.6 2.7 2.6 4.6 2.6h26.2c1.9 0 3.6-1 4.6-2.6l13-22.8c1-1.6 1-3.6.1-5.2z" fill="currentColor"/></svg>',D4='<svg width="18" height="18" viewBox="0 0 20 20" fill="none" aria-hidden="true" style="flex:none;opacity:.85"><rect x="2" y="5" width="16" height="12" rx="2" stroke="currentColor" stroke-width="1.5"/><path d="M2 9h16" stroke="currentColor" stroke-width="1.5"/><circle cx="6" cy="13" r="1" fill="currentColor"/></svg>',g0='<svg width="16" height="12" viewBox="0 0 16 12" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="%CLS%"><path d="M10,1l5,5l-5,5" stroke="currentColor" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><line x1="14" y1="6" x2="1" y2="6" stroke="currentColor" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',k4='<svg class="nq-cc-scan-glyph" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><g fill="currentColor"><path d="M1.21 7.06c.67 0 1.21-.54 1.21-1.21l-.04-3.12a.3.3 0 0 1 .3-.3H5.7a1.21 1.21 0 1 0 0-2.43H2.37A2.4 2.4 0 0 0 0 2.42v3.43c0 .67.54 1.21 1.21 1.21zM5.69 37.58H2.73a.3.3 0 0 1-.3-.3v-3.13a1.21 1.21 0 1 0-2.43 0v3.43A2.4 2.4 0 0 0 2.37 40H5.7a1.21 1.21 0 0 0 0-2.42zM38.79 32.94c-.67 0-1.21.54-1.21 1.21l.04 3.12a.3.3 0 0 1-.3.3H34.3a1.21 1.21 0 1 0 0 2.43h3.32A2.4 2.4 0 0 0 40 37.58v-3.43c0-.67-.54-1.21-1.21-1.21zM37.63 0H34.3a1.21 1.21 0 1 0 0 2.42h2.96c.17 0 .3.14.3.3v3.13a1.21 1.21 0 0 0 2.43 0V2.42A2.4 2.4 0 0 0 37.63 0z"/><path fill-rule="evenodd" clip-rule="evenodd" d="M13.94 15.15H6.67c-.67 0-1.22-.54-1.22-1.21V6.67c0-.67.55-1.21 1.22-1.21h7.27c.67 0 1.21.54 1.21 1.2v7.28c0 .67-.54 1.21-1.21 1.21zM8.18 7.88a.3.3 0 0 0-.3.3v4.24c0 .17.13.3.3.3h4.24a.3.3 0 0 0 .3-.3V8.18a.3.3 0 0 0-.3-.3H8.18zM6.67 24.85h7.27c.67 0 1.21.54 1.21 1.21v7.27c0 .67-.54 1.22-1.21 1.22H6.67c-.67 0-1.22-.55-1.22-1.22v-7.27c0-.67.55-1.21 1.22-1.21zm5.75 7.27a.3.3 0 0 0 .3-.3v-4.24a.3.3 0 0 0-.3-.3H8.18a.3.3 0 0 0-.3.3v4.24c0 .17.13.3.3.3h4.24zM26.06 5.45h7.27c.67 0 1.21.55 1.21 1.22v7.27c0 .67-.54 1.21-1.2 1.21h-7.28c-.67 0-1.21-.54-1.21-1.21V6.67c0-.67.54-1.22 1.21-1.22zm5.76 7.28a.3.3 0 0 0 .3-.3V8.17a.3.3 0 0 0-.3-.3h-4.24a.3.3 0 0 0-.3.3v4.24c0 .17.13.3.3.3h4.24z"/><path d="M17.58 10.6h1.2a.9.9 0 1 0 0-1.81.3.3 0 0 1-.3-.3V6.66a.9.9 0 1 0-1.81 0V9.7c0 .5.4.9.9.9zM21.21 7.58c.17 0 .3.13.3.3v6.66a.9.9 0 1 0 1.82 0V6.67c0-.5-.4-.91-.9-.91H21.2a.9.9 0 1 0 0 1.82zM12.42 18.18c0 .5.41.91.91.91h4.25c.5 0 .9-.4.9-.9v-4.86a.9.9 0 1 0-1.81 0v3.64a.3.3 0 0 1-.3.3h-3.04c-.5 0-.9.4-.9.91z"/><path d="M9.09 17.27c-.5 0-.9.4-.9.91v3.03a.3.3 0 0 1-.31.3H6.67a.9.9 0 1 0 0 1.82h15.75c.5 0 .91-.4.91-.9v-3.64a.9.9 0 0 0-1.82 0v2.42a.3.3 0 0 1-.3.3h-10.9a.3.3 0 0 1-.31-.3v-3.03c0-.5-.4-.9-.91-.9zM22.12 26.06c0-.5-.4-.9-.9-.9h-3.64c-.5 0-.91.4-.91.9v4.85a.9.9 0 1 0 1.81 0v-3.64c0-.16.14-.3.3-.3h2.43c.5 0 .91-.4.91-.9zM33.33 32.42h-10.3a.3.3 0 0 1-.3-.3V29.7a.9.9 0 1 0-1.82 0v3.63c0 .5.4.91.9.91h11.52a.9.9 0 0 0 0-1.82z"/><path fill-rule="evenodd" clip-rule="evenodd" d="M29.1 30h-3.65a.9.9 0 0 1-.9-.91v-3.64c0-.5.4-.9.9-.9h3.64c.5 0 .91.4.91.9v3.64c0 .5-.4.91-.9.91zm-2.43-3.64a.3.3 0 0 0-.3.3v1.22c0 .17.13.3.3.3h1.2a.3.3 0 0 0 .31-.3v-1.21a.3.3 0 0 0-.3-.3h-1.21z"/><path d="M32.73 20.9c-.5 0-.91.42-.91.92v7.88a.9.9 0 0 0 1.82 0v-7.88c0-.5-.41-.91-.91-.91zM33.64 17.58c0-.5-.41-.91-.91-.91h-6.67c-.5 0-.9.4-.9.9v3.64a.9.9 0 0 0 1.8 0V18.8c0-.17.15-.3.31-.3h5.46c.5 0 .9-.41.9-.91z"/></g></svg>',f4='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" aria-hidden="true"><g fill="none" stroke="currentColor" stroke-linecap="round" stroke-width="2.5px" stroke-linejoin="round"><path d="M40.25,23.25v-.5a6.5,6.5,0,0,0-6.5-6.5h-3.5a6.5,6.5,0,0,0-6.5,6.5v6.5a6.5,6.5,0,0,0,6.5,6.5h2"/><path d="M23.75,40.75v.5a6.5,6.5,0,0,0,6.5,6.5h3.5a6.5,6.5,0,0,0,6.5-6.5v-6.5a6.5,6.5,0,0,0-6.5-6.5h-2"/><line x1="32" y1="11.25" x2="32" y2="15.25"/><line x1="32" y1="48.75" x2="32" y2="52.75"/></g></svg>',T4=0;function h4(){let Q=`nq-cc-hex-${T4+=1}`;return`<svg class="nq-cc-hexlogo" viewBox="0 0 20 18" aria-hidden="true"><g fill="none"><path fill="url(#${Q})" d="M19.964 8.156 15.758.844A1.69 1.69 0 0014.299 0H5.887c-.6 0-1.156.32-1.456.844L.225 8.156c-.3.523-.3 1.165 0 1.688l4.206 7.312c.3.523.856.844 1.456.844h8.412c.6 0 1.156-.32 1.456-.844l4.206-7.312a1.69 1.69 0 00.003-1.688"/><defs><radialGradient id="${Q}" cx="0" cy="0" r="1" gradientTransform="matrix(20.1956 0 0 20.2552 15.188 17.766)" gradientUnits="userSpaceOnUse"><stop stop-color="#ec991c"/><stop offset="1" stop-color="#e9b213"/></radialGradient></defs></g></svg>`}function m0(Q,Z){let z=Q.address?U4(Q.address):null;if(z)return z;if(Q.label)return Q.label;let X=Q.address?.trim()??"";if(!X)return Z;return`${X.slice(0,7)}…${X.slice(-4)}`}function j(Q,Z,z){let X=document.createElement(Q);if(Z)X.className=Z;if(z)z.appendChild(X);return X}function u0(Q,Z){let{wallet:z,i18n:X}=Z,K=Z.languages??FQ,Y=Z.receive!==!1,V=!!Z.assets,W=typeof Z.getBalanceLuna==="function"||V,$=!!Z.fiat&&Z.fiat.currencies.length>0;if(Z.injectStyles!==!1)L4();let _=Z.fiat?.default??"USD";try{let G=localStorage.getItem(v0);if(G&&Z.fiat?.currencies.includes(G))_=G}catch{}if(Z.fiat&&!Z.fiat.currencies.includes(_))_=Z.fiat.currencies[0];let N=j("div","nq-cc");if(N.dataset.mode=!z||z.mode==="miniapp"?"miniapp":"hub",!z)N.dataset.face="lang";if(Z.network==="test")N.dataset.testnet="";Q.appendChild(N);let A=(G)=>K.find((I)=>I.id===G)??K[0],J=(G)=>O4[G.id]??G.name,P=j("button","nq-cc-face",N);P.type="button",P.setAttribute("aria-haspopup","menu"),P.setAttribute("aria-expanded","false");let H=j("button","nq-cc-face-flag",N);H.type="button",H.setAttribute("aria-haspopup","menu"),H.setAttribute("aria-expanded","false");function M(){P.textContent="";let G=z?.account??null;if(G){N.dataset.connected="";let I=j("span","nq-cc-face-icon",P);if(Z.identicon)I.appendChild(Z.identicon(G.address,28));else I.innerHTML=y0;let F=j("span","nq-cc-face-label",P);F.textContent=m0(G,X.t("shell.account"))}else{delete N.dataset.connected,P.insertAdjacentHTML("beforeend",D4);let I=j("span","nq-cc-face-label",P);I.textContent=X.t("shell.connectWallet")}P.insertAdjacentHTML("beforeend",gQ)}function C(){H.textContent="";let G=A(X.getLanguage());H.setAttribute("aria-label",X.t("shell.language")),H.appendChild(B(G.flag,{size:24})),H.insertAdjacentHTML("beforeend",gQ)}let O=j("div","nq-cc-menu",N);O.hidden=!0;let x=j("div","nq-cc-view-receive",O),U=j("div","nq-cc-view-send",O),f=j("div","nq-cc-view-main",O),EQ=[];function T(G,I){EQ.push([G,I]),G.textContent=X.t(I)}let ZQ=()=>{};function mQ(){for(let[G,I]of EQ)G.textContent=X.t(I);ZQ()}let zQ=j("div","nq-cc-section nq-cc-when-out nq-cc-when-hub",f),R=j("button","nq-cc-connect",zQ);if(R.type="button",T(R,"shell.connectWallet"),R.addEventListener("click",async()=>{if(!z)return;R.disabled=!0,R.textContent=X.t("shell.connecting");try{await z.connect(),q(!1),R.textContent=X.t("shell.connectWallet")}catch{R.textContent=X.t("shell.retry")}finally{R.disabled=!1}}),Z.onboard){let G=j("button","nq-cc-onboard",zQ);G.type="button",T(G,"shell.newToNimiq"),G.addEventListener("click",()=>Z.onboard())}j("div","nq-cc-divider nq-cc-when-out nq-cc-when-hub",f);let dQ=j("div","nq-cc-section nq-cc-wallet nq-cc-when-connected nq-cc-when-hub",f),lQ=j("div","nq-cc-account",dQ),uQ=j("span","nq-cc-identicon",lQ),AQ=j("button","nq-cc-name",lQ);AQ.type="button",AQ.addEventListener("click",()=>c0(AQ));let s=j("div","nq-cc-balance",lQ);s.hidden=!0;let SZ=j("span","nq-cc-balance-nim",s),XQ=j("span","nq-cc-balance-fiat",s),d=null;if(V)d=hZ(dQ,{assets:Z.assets,fiatTicker:()=>_,rate:$?(G)=>Z.fiat.rate(_,G):void 0,autoRefresh:!1,onSelect:Y?(G)=>rZ(G):void 0});function c0(G){if(G.querySelector("input"))return;let I=G.textContent??"";G.textContent="";let F=document.createElement("input");F.className="nq-cc-name-input",F.value=I,F.maxLength=24,G.appendChild(F),F.focus(),F.select();let D=()=>{let S=F.value.trim()||I;if(G.textContent=S,S!==I){if(z?.account)H4(z.account.address,S);if(M(),Z.onRename)Z.onRename(S)}};F.addEventListener("blur",D),F.addEventListener("keydown",(S)=>{if(S.key==="Enter")F.blur();if(S.key==="Escape")F.value=I,F.blur()})}{let G=j("div","nq-cc-actions",dQ);if(Y){let I=j("button","nq-cc-receive",G);I.type="button",I.insertAdjacentHTML("beforeend",g0.replace("%CLS%","nq-cc-arrow-down"));let F=j("span",void 0,I);T(F,"shell.receive"),I.addEventListener("click",()=>rZ())}{let I=j("button","nq-cc-send",G);I.type="button",I.insertAdjacentHTML("beforeend",g0.replace("%CLS%","nq-cc-arrow-up"));let F=j("span",void 0,I);T(F,"shell.send"),I.addEventListener("click",()=>{if(Z.send)q(!1),Z.send();else V1()})}if(Z.scan){let I=j("button","nq-cc-scan",G);I.type="button",I.setAttribute("aria-label","Scan QR code"),I.insertAdjacentHTML("beforeend",k4),I.addEventListener("click",()=>{q(!1),Z.scan()})}}if(j("div","nq-cc-divider nq-cc-when-connected nq-cc-when-hub",f),Z.createCashlink){let G=j("button","nq-cc-row nq-cc-when-connected nq-cc-when-hub",f);G.type="button";let I=j("span","nq-cc-cashlink-slot",G);I.innerHTML=f4;let F=j("span","nq-cc-strong",G);T(F,"shell.createCashlink"),G.addEventListener("click",()=>{q(!1),Z.createCashlink()}),j("div","nq-cc-divider nq-cc-when-connected nq-cc-when-hub",f)}let qZ=j("div","nq-cc-section",f),a=j("button","nq-cc-acc",qZ);a.type="button",a.setAttribute("aria-expanded","false");let p0=j("span","nq-cc-label",a);T(p0,"shell.language");let wZ=j("span","nq-cc-acc-value",a),bZ=j("span",void 0,wZ),i0=j("span","nq-cc-strong",wZ);a.insertAdjacentHTML("beforeend",gQ);let cQ=j("div","nq-cc-acc-body",qZ),n0=j("div","nq-cc-grid-wrap",cQ),pQ=j("div","nq-cc-grid nq-cc-cols-2",n0);pQ.setAttribute("role","listbox"),pQ.setAttribute("aria-label",X.t("shell.language"));let RZ=new Map;for(let G of K){let I=j("button","nq-cc-card",pQ);I.type="button",I.setAttribute("role","option"),j("span","nq-cc-card-art",I).appendChild(B(G.flag,{size:26}));let D=j("span","nq-cc-card-name",I);D.textContent=J(G),I.addEventListener("click",()=>{X.setLanguage(G.id),window.setTimeout(()=>eQ(a,cQ),260)}),RZ.set(G.id,I)}function vZ(){let G=A(X.getLanguage());bZ.textContent="",bZ.appendChild(B(G.flag,{size:24})),i0.textContent=J(G);for(let[I,F]of RZ){let D=I===G.id;F.classList.toggle("nq-cc-current",D),F.setAttribute("aria-selected",String(D))}}let xQ=null,iQ=null,BZ=new Map;if($){let G=W?" nq-cc-when-connected":"";j("div",`nq-cc-divider${G}`,f);let I=j("div",`nq-cc-section${G}`,f),F=j("button","nq-cc-acc",I);F.type="button",F.setAttribute("aria-expanded","false");let D=j("span","nq-cc-label",F);T(D,"shell.amountsIn");let S=j("span","nq-cc-acc-value",F);xQ=j("span",void 0,S),iQ=j("span","nq-cc-strong",S),F.insertAdjacentHTML("beforeend",gQ);let g=j("div","nq-cc-acc-body",I),QZ=j("div","nq-cc-grid-wrap",g),_Q=j("div","nq-cc-grid nq-cc-cols-3",QZ);_Q.setAttribute("role","listbox"),_Q.setAttribute("aria-label",X.t("shell.amountsIn"));for(let e of Z.fiat.currencies){let YQ=j("button","nq-cc-card",_Q);YQ.type="button",YQ.setAttribute("role","option");let $1=j("span","nq-cc-card-art",YQ),z0=B0[e];if(z0)$1.appendChild(B(z0,{size:26}));let j1=j("span","nq-cc-card-ticker",YQ);j1.textContent=e,YQ.addEventListener("click",()=>{if(e===_){window.setTimeout(()=>eQ(F,g),260);return}_=e;try{localStorage.setItem(v0,e)}catch{}if(yZ(),W)jQ(!0);Z.fiat.onChange?.(e),window.setTimeout(()=>eQ(F,g),260)}),BZ.set(e,YQ)}oZ(F,g)}function yZ(){if(!xQ||!iQ)return;xQ.textContent="";let G=B0[_];if(G)xQ.appendChild(B(G,{size:24}));iQ.textContent=_;for(let[I,F]of BZ){let D=I===_;F.classList.toggle("nq-cc-current",D),F.setAttribute("aria-selected",String(D))}}if(Z.openInPay){j("div","nq-cc-divider nq-cc-when-hub",f);let G=j("button","nq-cc-row nq-cc-when-hub",f);G.type="button",G.insertAdjacentHTML("beforeend",h4());let I=j("span","nq-cc-strong",G);T(I,"shell.openInPay"),G.addEventListener("click",()=>{q(!1);let F=Z.openInPay;window.location.href=typeof F==="function"?F():F})}if(Z.reportBug){if(typeof Z.reportBug==="object"&&Z.reportBug.bot)LZ(Z.reportBug.bot.service??"https://bot.nimiq.tech");j("div","nq-cc-divider",f);let G=j("button","nq-cc-row nq-cc-report",f);G.type="button",j("span","nq-cc-cashlink-slot",G).insertAdjacentHTML("beforeend",fZ);let F=j("span","nq-cc-strong",G);T(F,"shell.reportBug"),G.addEventListener("click",()=>{q(!1);let D=Z.reportBug;if(typeof D==="function")D();else TZ(document,X,D)})}if(z&&Z.switchAccount!==!1){let G=j("button","nq-cc-row nq-cc-when-connected nq-cc-when-hub",f);G.type="button";let I=j("span","nq-cc-strong",G);T(I,"shell.switchAccount"),G.addEventListener("click",async()=>{q(!1);try{await z.connect()}catch{}})}j("div","nq-cc-divider nq-cc-footer-divider",f);let gZ=j("div","nq-cc-footer",f),nQ=j("button","nq-cc-disconnect nq-cc-when-connected nq-cc-when-hub",gZ);nQ.type="button",T(nQ,"shell.disconnect"),nQ.addEventListener("click",()=>{z?.disconnect(),q(!1)});let mZ=j("span","nq-cc-net-group",gZ),r0=j("span",void 0,mZ);T(r0,"shell.network");let o0=j("span","nq-cc-badge",mZ);o0.textContent="Testnet";let UQ=j("button","nq-cc-back",x);UQ.type="button",UQ.appendChild(document.createTextNode("‹ "));let dZ=j("span","nq-cc-strong",UQ);T(dZ,"shell.receive"),UQ.addEventListener("click",()=>N.classList.remove("nq-cc-show-receive")),j("div","nq-cc-divider",x);let HQ=j("div","nq-cc-receive-body",x),rQ=j("div","nq-cc-qr",HQ),GQ=j("span","nq-cc-copy-wrap",HQ),u=j("button","nq-cc-address",GQ);u.type="button",u.title=X.t("shell.copyAddress");let lZ=j("span","nq-cc-copy-tooltip",GQ);lZ.setAttribute("aria-hidden","true"),T(lZ,"shell.copied");let s0=j("p","nq-cc-receive-hint",HQ);T(s0,"shell.tapToCopy");let oQ=j("p","nq-cc-net-warn",HQ);oQ.hidden=!0;let sQ=null,uZ;u.addEventListener("click",()=>{let G=sQ??z?.account?.address;if(!G)return;try{navigator.clipboard.writeText(G)}catch{}GQ.classList.add("nq-cc-copied","nq-cc-copied-hold"),clearTimeout(uZ),uZ=setTimeout(()=>GQ.classList.remove("nq-cc-copied"),800)}),u.addEventListener("blur",()=>GQ.classList.remove("nq-cc-copied-hold"));let OQ=j("button","nq-cc-back",U);OQ.type="button",OQ.appendChild(document.createTextNode("‹ "));let a0=j("span","nq-cc-strong",OQ);T(a0,"shell.send"),OQ.addEventListener("click",()=>tQ()),j("div","nq-cc-divider",U);let c=j("div","nq-cc-send-body",U),t0=j("label","nq-cc-field-label",c);T(t0,"shell.recipient");let y=j("input","nq-cc-input nq-cc-input-addr",c);y.placeholder="NQ00 0000 0000 0000 0000 0000 0000 0000 0000",y.autocomplete="off",y.spellcheck=!1;let LQ=j("div","nq-cc-contacts",c);LQ.hidden=!0;async function e0(G){let I=Z.contacts?.add;if(!I)return;let F=G.replace(/\s+/g,"").toUpperCase();try{if((await Z.contacts.list()??[]).some((g)=>g.address.replace(/\s+/g,"").toUpperCase()===F))return;let S=window.prompt(X.t("shell.saveContact"))?.trim();if(!S)return;await I({label:S,address:G,asset:"NIM"})}catch{}}async function Q1(){if(!Z.contacts)return;let G=[];try{G=await Z.contacts.list()??[]}catch{G=[]}let I=G.filter((F)=>(F.asset??"NIM")==="NIM");LQ.textContent="",LQ.hidden=I.length===0;for(let F of I){let D=j("button","nq-cc-contact",LQ);D.type="button",D.textContent=F.label,D.title=F.address,D.addEventListener("click",()=>{y.value=F.address,$Q(),y.focus(),y.setSelectionRange(0,0),y.scrollLeft=0})}}let Z1=j("label","nq-cc-field-label",c);T(Z1,"shell.amount");let cZ=j("div","nq-cc-amount-row",c),KQ=j("input","nq-cc-input",cZ);KQ.placeholder="0",KQ.inputMode="decimal",KQ.autocomplete="off";let z1=j("span","nq-cc-amount-suffix",cZ);z1.textContent="NIM";let X1=j("p","nq-cc-send-hint",c),aQ=j("p","nq-cc-send-error",c),t=j("button","nq-cc-send-confirm",c);t.type="button",T(t,"shell.send");let pZ=j("div","nq-cc-send-done",U);pZ.insertAdjacentHTML("beforeend",'<svg width="36" height="36" viewBox="0 0 24 24" fill="none" aria-hidden="true"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2"/><path d="M7.5 12.5l3 3 6-6.5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" fill="none"/></svg>');let K1=j("span",void 0,pZ);T(K1,"shell.sent");let Y1=/^NQ\d{2}[0-9A-HJ-NP-VXY]{32}$/,iZ=()=>y.value.toUpperCase().replace(/[\s-]+/g,""),nZ=()=>{let G=Number(KQ.value.replace(",","."));return Number.isFinite(G)?G:0};function $Q(){let G=Y1.test(iZ()),I=nZ(),F=I>0&&(v===null||I<=vQ(v));t.disabled=!(G&&F)}y.addEventListener("input",$Q),KQ.addEventListener("input",$Q);function V1(){if(!z?.account)return;aQ.textContent="",U.classList.remove("nq-cc-sent"),X1.textContent=v!==null?`${X.t("shell.available")}: ${RQ(v)} NIM`:"",$Q(),N.classList.add("nq-cc-show-send"),y.focus(),Q1()}function tQ(){N.classList.remove("nq-cc-show-send")}t.addEventListener("click",async()=>{if(!z)return;let I=iZ().replace(/(.{4})(?=.)/g,"$1 ");t.disabled=!0,aQ.textContent="",t.textContent=X.t("shell.sending");try{if(await z.pay({recipient:I,valueLuna:NZ(nZ())}))U.classList.add("nq-cc-sent"),e0(I),y.value="",KQ.value="",kQ=0,window.setTimeout(()=>{tQ(),jQ(!0)},1800);else tQ()}catch(F){if(!/cancel|denied|rejected|closed|dismiss/i.test(String(F)))aQ.textContent=X.t("shell.sendFailed")}finally{t.textContent=X.t("shell.send"),$Q()}});let DQ="",p=null;ZQ=()=>{dZ.textContent=p?`${X.t("shell.receive")} ${p.ticker}`:X.t("shell.receive"),oQ.hidden=!p,oQ.textContent=p?X.t("shell.networkOnly",{ticker:p.ticker,network:p.network}):""};function rZ(G){let I=z?.account??null;if(!I)return;if(G&&!G.address)return;let F=G?.address??I.address;if(!F)return;N.classList.add("nq-cc-show-receive");let D=F.replace(/\s+/g,"");sQ=D,u.textContent="";let S=d0(D);u.style.setProperty("--nq-cc-addr-cols",String(S.columns));for(let QZ of S.cells){let _Q=j("span",void 0,u);_Q.textContent=QZ}p=G??null,ZQ();let g=G?G.uri?.(D)??D:`nimiq:${D}`;if(Z.qr&&DQ!==g)rQ.textContent="",rQ.appendChild(Z.qr(g,164)),DQ=g}function oZ(G,I){G.addEventListener("click",()=>{let F=I.classList.toggle("nq-cc-open");G.setAttribute("aria-expanded",String(F))})}function eQ(G,I){I.classList.remove("nq-cc-open"),G.setAttribute("aria-expanded","false")}oZ(a,cQ);let kQ=0,v=null;async function jQ(G=!1){let I=z?.account??null;if(!W||!I)return;if(d){await d.refresh(G);let D=d.total();s.hidden=D===null,SZ.textContent=D===null?"":WQ(D,_),XQ.hidden=!0;let S=d.units("NIM");if(S!==null)v=Number(S);if(!Z.getBalanceLuna)return}let F=Date.now();if(!G&&F-kQ<30000&&v!==null){sZ();return}try{v=await Z.getBalanceLuna(I.address),kQ=F}catch{}if(sZ(),$&&!d&&v!==null)try{let D=await Z.fiat.rate(_);if(D!==null&&z?.account)XQ.textContent=WQ(vQ(v)*D,_),XQ.hidden=!1;else XQ.hidden=!0}catch{XQ.hidden=!0}}function sZ(){if(d)return;if(v===null){s.hidden=!0;return}if(s.hidden=!1,SZ.textContent=`${RQ(v)} NIM`,!$)XQ.hidden=!0}function aZ(){let G=z?.account??null;if(!G)return;if(uQ.textContent="",Z.identicon)uQ.appendChild(Z.identicon(G.address,40));else uQ.innerHTML=y0;AQ.textContent=m0(G,X.t("shell.account")),DQ="",jQ()}function q(G){if(O.hidden=!G,P.setAttribute("aria-expanded",String(G)),H.setAttribute("aria-expanded",String(G)),G)document.addEventListener("click",tZ,!0),document.addEventListener("keydown",eZ),jQ();else N.classList.remove("nq-cc-show-receive"),N.classList.remove("nq-cc-show-send"),document.removeEventListener("click",tZ,!0),document.removeEventListener("keydown",eZ)}function tZ(G){if(!N.contains(G.target))q(!1)}function eZ(G){if(G.key==="Escape")q(!1)}let Q0=()=>q(O.hidden);if(P.addEventListener("click",Q0),H.addEventListener("click",Q0),M(),C(),vZ(),yZ(),$)Z.fiat.onChange?.(_);if(z?.account)aZ();let Z0=z?.account?.address??null,W1=z?z.onAccountChange(()=>{let G=z.account?.address??null;if(G!==Z0)Z0=G,v=null,kQ=0,d?.clear(),N.classList.remove("nq-cc-show-receive"),p=null,sQ=null,DQ="",u.textContent="",rQ.textContent="";if(M(),z.account)aZ(),jQ(!0);else s.hidden=!0}):()=>{},G1=X.onChange(()=>{mQ(),vZ(),C(),M()});return{el:N,get fiatTicker(){return $?_:null},open:()=>q(!0),close:()=>q(!1),destroy(){W1(),G1(),q(!1),d?.destroy(),N.remove()}}}var S4=u0;export{S0 as validateFeedbackInput,h0 as submitToBot,q0 as submitFeedback,r1 as shellLocales,CQ as scrubAddresses,l1 as parseNim,DZ as pageContext,TZ as openReportBugSheet,NZ as nimToLuna,P4 as mountWalletPill,UZ as mountProfileWidget,u0 as mountMiniWallet,Y4 as mountLanguageSwitcher,G4 as mountLanguagePill,S4 as mountCornerControl,hZ as mountAssetList,o1 as mergeLocales,vQ as lunaToNim,ZZ as isMiniAppHost,LZ as installReportCapture,zZ as hasNimiqProvider,bQ as fmtUnits,RQ as fmtNim,WQ as fmtFiat,EZ as flagDataUrl,BQ as donutPoint,Q4 as donutArcs,fQ as detectModeSync,R1 as createWallet,n1 as createI18n,w0 as collectDiagnostics,B as buildFlagHex,e1 as areaPaths,d0 as addressGrid,CZ as SHELL_LANGUAGES,B1 as NIM_DECIMALS,PQ as MiniAppBackend,v1 as LUNA_PER_NIM,MQ as HubBackend,E0 as FLAG_SVG,AZ as FLAG_FIT,FQ as FEATURED_LANGUAGES};
